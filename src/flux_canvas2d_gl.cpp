...
void Canvas2D::setCompositeOp(CompositeOp op)
{
    ...
    switch (op)
    {
    case CompositeOp::SourceOver: glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); break;
    case CompositeOp::Copy:       glBlendFunc(GL_ONE, GL_ZERO); break;
...
void Canvas2D::strokeRoundedRect(float x, float y, float w, float h, float r)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        p4(ax-nx, ay-ny); p4(ax+nx, ay+ny); p4(bx+nx, by+ny);
        p4(bx+nx, by+ny); p4(bx-nx, by-ny); p4(ax-nx, ay-ny);
    }
...
void Canvas2D::fillCircle(float cx, float cy, float r)
{
    ...
    std::vector<float> v;
    v.push_back(cx); v.push_back(cy); v.push_back(0); v.push_back(0);
    for (int i = 0; i <= segs; ++i)
    {
        float a = float(i) / float(segs) * 2.f * float(M_PI);
        v.push_back(cx + cosf(a) * r); v.push_back(cy + sinf(a) * r);
...
void Canvas2D::strokeCircle(float cx, float cy, float r)
{
    ...
    for (int i = 0; i <= segs; ++i)
    {
        ...
        strip.push_back((r - lw) * ca + cx); strip.push_back((r - lw) * sa + cy);
...
void Canvas2D::bezierCurveTo(float c1x, float c1y, float c2x, float c2y, float x, float y)
{
    ...
    for (int i = 1; i <= n; ++i)
    {
        ...
        path_.push_back({mt*mt*mt*curX_ + 3*mt*mt*t*c1x + 3*mt*t*t*c2x + t*t*t*x,
                         mt*mt*mt*curY_ + 3*mt*mt*t*c1y + 3*mt*t*t*c2y + t*t*t*y, false});
    }
    curX_ = x; curY_ = y;
}
...
void Canvas2D::fillText(const std::string &text, float x, float y, float /*maxW*/)
{
    ...
    else if (textBaseline_ == TextBaseline::Middle) py = y + ascender * 0.5f;
    else if (textBaseline_ == TextBaseline::Bottom) py = y + descent_u * scale;
...
void Canvas2D::strokeText(const std::string &text, float x, float y, float maxW)
{
    ...
    fillText(text, x - 1, y, maxW); fillText(text, x + 1, y, maxW);
    fillText(text, x, y - 1, maxW); fillText(text, x, y + 1, maxW);
    setFillColor(saved);
