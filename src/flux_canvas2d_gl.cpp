                           ...
                           GlyphEntry &out)
{
    ...
    for (int row = 0; row < gh; ++row)
    {
        ...
        unsigned char       *dst = atlasPixels.data()
                                   + (shelfY + row) * atlasW + shelfX;
                                  ...
                                  int segs, std::vector<float> &out)
{
    ...
    auto corner = [&](float ox, float oy, float startA)
    {
        for (int i = 0; i <= segs; ++i)
        {
            ...
            out.push_back(ox+cosf(a)*r); out.push_back(oy+sinf(a)*r);
    ...
    out.push_back(x+w-r+cosf(a)*r); out.push_back(y+r+sinf(a)*r);
    out.push_back(0); out.push_back(0);
}
...
void Canvas2D::strokeRoundedRect(float x, float y, float w, float h, float r)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        push4(ax-nx,ay-ny); push4(ax+nx,ay+ny); push4(bx+nx,by+ny);
        push4(bx+nx,by+ny); push4(bx-nx,by-ny); push4(ax-nx,ay-ny);
    }
...
void Canvas2D::fillPath()
{
    ...
    for (auto &p:path_)
    {
        verts.push_back(p.x); verts.push_back(p.y);
        verts.push_back(0); verts.push_back(0);
    }
...
{
    auto *img = new Canvas2DImage();
    img->texId=texId; img->width=w; img->height=h;
    return img;
}
...
{
    float x0=x, y0=y; ctm_.apply(x0,y0);
    float x1=x+w, y1=y+h; ctm_.apply(x1,y1);
