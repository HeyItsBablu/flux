...
// Compiled on all non-Win32 platforms (Linux, Web, Android, macOS).
...
// macOS stubs are defined in flux_canvas2d.hpp for Apple
#endif
                           ...
                           GlyphEntry &out)
{
    ...
    int gw = (int)bitmap.width, gh = (int)bitmap.rows;
...
// ─────────────────────────────────────────────────────────────────────────────
// buildMVP  — combines backend_->mvp (set by CanvasWidget) with ctm_
//
// This is Option A: the base projection lives in the backend struct.
// CanvasWidget writes it once per frame before constructing Canvas2D.
// ─────────────────────────────────────────────────────────────────────────────
void Canvas2D::buildMVP(float out[16]) const
{
    // ctm_ as column-major 4×4
    float c[16] = {
        ctm_.m[0], ctm_.m[3], 0, 0,
        ctm_.m[1], ctm_.m[4], 0, 0,
        0, 0, 1, 0,
        ctm_.m[2], ctm_.m[5], 0, 1};
    const float *base = backend_->mvp; // ← Option A: read from backend
    memset(out, 0, 64);
...
void Canvas2D::setLineCap(LineCap) { /* approximated in GL */ }
void Canvas2D::setLineJoin(LineJoin) { /* approximated in GL */ }
...
void Canvas2D::drawTexturedQuad(unsigned int tex,
...
{
    Color col = (fillIsGrad_ && !gStops_.empty())
                    ? Color{uint8_t((gStops_.front().second.r + gStops_.back().second.r) / 2),
                            uint8_t((gStops_.front().second.g + gStops_.back().second.g) / 2),
                            uint8_t((gStops_.front().second.b + gStops_.back().second.b) / 2),
                            uint8_t((gStops_.front().second.a + gStops_.back().second.a) / 2)}
                    : fillColor_;
    float v[] = {x, y, 0, 0, x + w, y, 1, 0, x + w, y + h, 1, 1,
                 x + w, y + h, 1, 1, x, y + h, 0, 1, x, y, 0, 0};
    uploadAndDraw(v, 6, GL_TRIANGLES, col);
}
...
void Canvas2D::strokeRect(float x, float y, float w, float h)
{
    ...
    {
        float v[] = {ex, ey, 0, 0, ex + ew, ey, 1, 0, ex + ew, ey + eh, 1, 1,
                     ex + ew, ey + eh, 1, 1, ex, ey + eh, 0, 1, ex, ey, 0, 0};
        uploadAndDraw(v, 6, GL_TRIANGLES, strokeColor_);
    };
    edge(x, y, w, lw);
    edge(x, y + h - lw, w, lw);
    edge(x, y + lw, lw, h - lw * 2);
    edge(x + w - lw, y + lw, lw, h - lw * 2);
}
...
{
    std::vector<float> v;
    buildRoundedRectVerts(x, y, w, h, r, 8, v);
    uploadAndDraw(v.data(), int(v.size() / 4), GL_TRIANGLE_FAN, fillColor_);
}
...
void Canvas2D::strokeRoundedRect(float x, float y, float w, float h, float r)
{
    ...
    {
        int j = (i + 1) % n;
        float ax = loop[i * 2], ay = loop[i * 2 + 1], bx = loop[j * 2], by = loop[j * 2 + 1];
        float dx = bx - ax, dy = by - ay, len = sqrtf(dx * dx + dy * dy);
        if (len < 1e-5f)
            continue;
        float nx = -dy / len * lw, ny = dx / len * lw;
        auto p4 = [&](float x_, float y_)
        { strip.push_back(x_);strip.push_back(y_);strip.push_back(0);strip.push_back(0); };
        p4(ax - nx, ay - ny);
        p4(ax + nx, ay + ny);
        p4(bx + nx, by + ny);
        p4(bx + nx, by + ny);
...
void Canvas2D::fillCircle(float cx, float cy, float r)
{
    ...
    for (int i = 0; i <= segs; ++i)
    {
        ...
        v.push_back(cx + cosf(a) * r);
        v.push_back(cy + sinf(a) * r);
        v.push_back(0);
        v.push_back(0);
    }
    uploadAndDraw(v.data(), int(v.size() / 4), GL_TRIANGLE_FAN, fillColor_);
                             ...
                             float a0, float a1, bool ccw, bool move)
{
    ...
    for (int i = 0; i <= segs; ++i)
    {
        ...
        path_.push_back({cx + cosf(a) * r, cy + sinf(a) * r, i == 0 && move});
    }
}
...
void Canvas2D::strokePath()
{
    ...
    {
        float ax = path_[i].x, ay = path_[i].y, bx = path_[i + 1].x, by = path_[i + 1].y;
        float dx = bx - ax, dy = by - ay, len = sqrtf(dx * dx + dy * dy);
        if (len < 1e-5f)
            continue;
        float nx = -dy / len * lw, ny = dx / len * lw;
...
void Canvas2D::arc(float cx, float cy, float r, float a0, float a1, bool ccw) { tessellateArc(cx, cy, r, a0, a1, ccw, false); }
...
void Canvas2D::quadraticCurveTo(float cpx, float cpy, float x, float y)
{
    ...
    for (int i = 1; i <= n; ++i)
    {
        ...
        path_.push_back({mt * mt * curX_ + 2 * mt * t * cpx + t * t * x, mt * mt * curY_ + 2 * mt * t * cpy + t * t * y, false});
    }
    curX_ = x;
    curY_ = y;
}
void Canvas2D::bezierCurveTo(float c1x, float c1y, float c2x, float c2y, float x, float y)
{
    ...
    {
        float t = float(i) / n, mt = 1 - t;
        path_.push_back({mt * mt * mt * curX_ + 3 * mt * mt * t * c1x + 3 * mt * t * t * c2x + t * t * t * x, mt * mt * mt * curY_ + 3 * mt * mt * t * c1y + 3 * mt * t * t * c2y + t * t * t * y, false});
    }
    curX_ = x;
    curY_ = y;
}
...
void Canvas2D::ellipse(float cx, float cy, float rx, float ry, float rot, float a0, float a1, bool ccw)
{
    int segs = 32;
    bool first = true;
    float step = (a1 - a0) / float(segs);
    ...
    for (int i = 0; i <= segs; ++i)
    {
        ...
        path_.push_back({cosf(rot) * px - sinf(rot) * py + cx, sinf(rot) * px + cosf(rot) * py + cy, first});
        first = false;
    }
}
...
void Canvas2D::pushClipRect(float x, float y, float w, float h)
{
    ...
    float px0 = toPixX(x0), py0 = toPixY(y0), px1 = toPixX(x1), py1 = toPixY(y1);
    if (px0 > px1)
        std::swap(px0, px1);
    if (py0 > py1)
        std::swap(py0, py1);
...
void Canvas2D::updateImage(Canvas2DImage *img, const unsigned char *rgba, int w, int h)
{
    if (!img)
        return;
    auto *gl_img = static_cast<Canvas2DImageGL *>(img);
    if (!gl_img->texId)
        return;
    glBindTexture(GL_TEXTURE_2D, gl_img->texId);
...
void Canvas2D::drawImage(const Canvas2DImage *img, float dx, float dy)
{
    if (!img)
        return;
    const auto *gi = static_cast<const Canvas2DImageGL *>(img);
    if (!gi->texId)
        return;
    drawTexturedQuad(gi->texId, 0, 0, float(img->width), float(img->height),
                     dx, dy, float(img->width), float(img->height), img->width, img->height);
}
                         ...
                         float dx, float dy, float dw, float dh)
{
    ...
    if (!gi->texId)
        return;
    drawTexturedQuad(gi->texId, sx, sy, sw, sh, dx, dy, dw, dh, img->width, img->height);
}
...
int Canvas2D::resolveFont() const
{
    ...
    {
        int i = gl->findFont(fontFace_ + "-bold-italic");
        if (i >= 0)
            return i;
    }
    if (fontBold_)
    {
        int i = gl->findFont(fontFace_ + "-bold");
        if (i >= 0)
            return i;
    }
    if (fontItalic_)
    {
        int i = gl->findFont(fontFace_ + "-italic");
        if (i >= 0)
            return i;
    }
...
void Canvas2D::fillText(const std::string &text, float x, float y, float /*maxW*/)
{
    ...
    FT_Face face = gl->fonts[fontIdx].ftFace;
    ...
    for (char ch : text)
    {
        auto *g = gl->getGlyph(fontIdx, (unsigned char)ch, ps);
        if (g)
    ...
    if (textBaseline_ == TextBaseline::Top)
        ...
        py = y + float(face->descender) * scale;
    ...
    glUniform4f(gl->flatColor, fillColor_.r / 255.f, fillColor_.g / 255.f, fillColor_.b / 255.f, a);
    glUniform1i(glGetUniformLocation(gl->flatProg, "uTex"), 0);
    glActiveTexture(GL_TEXTURE0);
    ...
    for (char ch : text)
    {
        ...
        if (g->glyphW > 0 && g->glyphH > 0)
        {
            ...
            float u0 = float(g->atlasX) / float(gl->atlasW), v0 = float(g->atlasY) / float(gl->atlasH);
            float u1 = float(g->atlasX + g->glyphW) / float(gl->atlasW), v1 = float(g->atlasY + g->glyphH) / float(gl->atlasH);
            float verts[] = {gx, gy, u0, v0, gx + g->glyphW, gy, u1, v0, gx + g->glyphW, gy + g->glyphH, u1, v1,
                             gx + g->glyphW, gy + g->glyphH, u1, v1, gx, gy + g->glyphH, u0, v1, gx, gy, u0, v0};
            glBindBuffer(GL_ARRAY_BUFFER, gl->vbo);
            glBufferData(GL_ARRAY_BUFFER, sizeof(verts), verts, GL_DYNAMIC_DRAW);
            glDrawArrays(GL_TRIANGLES, 0, 6);
        }
...
float Canvas2D::measureText(const std::string &text)
{
    ...
    for (char ch : text)
    {
        auto *g = backend_->gl->getGlyph(fontIdx, (unsigned char)ch, ps);
        if (g)
