...
#define AUDIO_LOGI(fmt, ...) \
    do { char _b[256]; snprintf(_b,256,"[FluxAudio] " fmt "\n",##__VA_ARGS__); \
...
struct FluxAudio::Impl {
    ...
    // ── XAudio2 init ──────────────────────────────────────────────────────────
    bool ensureEngine() {
        if (xaudio2) return true;
        CoInitializeEx(nullptr, COINIT_MULTITHREADED);
        HRESULT hr = XAudio2Create(xaudio2.GetAddressOf(), 0);
        if (FAILED(hr)) { AUDIO_LOGE("XAudio2Create: 0x%08X", hr); return false; }
        hr = xaudio2->CreateMasteringVoice(&masterVoice);
        if (FAILED(hr)) { AUDIO_LOGE("CreateMasteringVoice: 0x%08X", hr); return false; }
        return true;
    }

    bool createSourceVoice(int sampleRate) {
        if (sourceVoice) { sourceVoice->DestroyVoice(); sourceVoice = nullptr; }
        ...
        if (FAILED(hr)) {
            ...
            sourceVoice = nullptr; return false;
        }
    ...
    void streamFeedLoop() {
        for (int i = 0; i < 2; i++) {
            int got = streamCallback
                    ? streamCallback(streamBufs[i], kStreamFrames) : 0;
            if (got == 0) memset(streamBufs[i], 0, sizeof(float)*kStreamFrames);
        ...
        while (streamRunning.load()) {
            XAUDIO2_VOICE_STATE vs{};
            for (;;) {
                if (!sourceVoice || !streamRunning.load()) goto done;
                sourceVoice->GetState(&vs);
                if (vs.BuffersQueued < 2) break;
            ...
            int got = streamCallback
                    ? streamCallback(streamBufs[back], kStreamFrames) : 0;
            if (got == 0)
...
void  FluxAudio::setVolume(float v) {
    ...
    if (m_impl->sourceVoice) m_impl->sourceVoice->SetVolume(m_impl->volume.load());
}
...
void FluxAudio::pause() {
    ...
    if (m_impl->sourceVoice) m_impl->sourceVoice->Stop();
...
void FluxAudio::resume() {
    ...
    m_impl->paused  = false;
    m_impl->playing = true;
    if (m_impl->sourceVoice) m_impl->sourceVoice->Start();
}
...
bool FluxAudio::playPCM(const std::vector<float>& samples, int sampleRate) {
    ...
    if (!m_impl->ensureEngine()) return false;
    if (!m_impl->createSourceVoice(sampleRate)) return false;
    m_impl->submitAndStart();
    m_impl->playing = true;
    return true;
}
...
bool FluxAudio::playStream(StreamCallback cb, int sampleRate) {
    ...
    if (!m_impl->createSourceVoice(sampleRate)) {
        m_impl->playing = false; return false;
    }
...
void FluxAudio::closePlayback() {
    ...
    if (m_impl->streamThread.joinable()) m_impl->streamThread.join();
