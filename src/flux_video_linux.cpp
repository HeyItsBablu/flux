...
static void processAudioFrame(AVFrame* frame) {
    ...
    for (int i = 0; i < frames; i++) {
        ...
        for (int c = 0; c < ch; c++) {
            if (isFloat) {
                if (isPlanar) sum += ((const float*)frame->data[c])[i];
                else          sum += ((const float*)frame->data[0])[i * ch + c];
            } else if (isS16) {
                if (isPlanar) sum += ((const int16_t*)frame->data[c])[i] / 32768.f;
                else          sum += ((const int16_t*)frame->data[0])[i * ch + c] / 32768.f;
            }
        }
...
static void decodeLoop(std::string path) {
    ...
    if (avformat_open_input(&fmtCtx, path.c_str(), nullptr, nullptr) < 0) {
        s.state = FluxVideo::State::Error; return;
    }
    if (avformat_find_stream_info(fmtCtx, nullptr) < 0) {
        avformat_close_input(&fmtCtx); s.state = FluxVideo::State::Error; return;
    }
    ...
    if (fmtCtx->duration != AV_NOPTS_VALUE) s.durationUs = fmtCtx->duration;
    ...
    if (avcodec_open2(vCtx, vCodec, nullptr) < 0) {
        avcodec_free_context(&vCtx); avformat_close_input(&fmtCtx);
        s.state = FluxVideo::State::Error; return;
    }

    int vidW = vCtx->width, vidH = vCtx->height;
    s.videoWidth = vidW; s.videoHeight = vidH;
    ...
    if (audioStreamIdx >= 0) {
        ...
        if (avcodec_open2(aCtx, aCodec, nullptr) == 0) {
            ...
            avcodec_free_context(&aCtx); aCtx = nullptr;
        }
    }
    ...
    {
        std::lock_guard<std::mutex> lk(s.frameMutex);
        s.frameWidth  = vidW; s.frameHeight = vidH;
