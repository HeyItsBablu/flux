...
static Canvas2DGL *acquireSharedGL() {
    if (!s_sharedGL) {
        ...
        reg("sans",             "/fonts/segoeui.ttf");
        reg("sans-bold",        "/fonts/segoeuib.ttf");
        reg("sans-italic",      "/fonts/segoeuii.ttf");
        reg("sans-bold-italic", "/fonts/segoeuiz.ttf");
        reg("mono",             "/fonts/consola.ttf");
        reg("mono-bold",        "/fonts/consolab.ttf");
    }
    ++s_sharedGLUsers;
    return s_sharedGL;
}
...
static void tickAndRender(CanvasWidget *w) {
    auto &s = webState(w);
    if (!s.initialized || !w->canvasGL_) return;
    ...
    // w->x / w->y are in logical pixels; the web painter works in physical
    // pixels (dpr-scaled). We need physical coords for the GL viewport and
    // scissor to align with the Canvas 2D overlay.
    double dpr = EM_ASM_DOUBLE({ return Module._fluxDPR || 1.0; });
    int physX = (int)(w->x * dpr);
    int physY = (int)(w->y * dpr);
    int physW = (int)(w->width  * dpr);
    int physH = (int)(w->height * dpr);
