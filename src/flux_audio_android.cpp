...
struct FluxAudio::Impl {
            ...
            AAudioStream*, void* userData, void* audioData, int32_t numFrames)
    {
        ...
        if (self->playCallback) {
            ...
            if (written == 0) { self->playing = false; finished = true; }
        } else {
            ...
            AAudioStream*, void* userData, void* audioData, int32_t numFrames)
    {
        ...
        if (!self->recording) return AAUDIO_CALLBACK_RESULT_STOP;

        float rms = 0.f;
        for (int i = 0; i < numFrames; i++) rms += input[i] * input[i];
        self->inputLevel.store(std::sqrt(rms / numFrames));

        if (self->recCallback) self->recCallback(input, numFrames);
        return AAUDIO_CALLBACK_RESULT_CONTINUE;
    }
};
...
bool FluxAudio::openPlayback(int sampleRate, int channels) {
    if (m_impl->playStream) closePlayback();
...
bool FluxAudio::playStream(StreamCallback cb, int sampleRate) {
    ...
    if (!m_impl->playStream && !openPlayback(sampleRate)) return false;
    return startPlayback();
}

bool FluxAudio::startPlayback() {
    if (!m_impl->playStream) return false;
    m_impl->playing = true;
...
void FluxAudio::stopPlayback() {
    m_impl->playing = false;
    if (m_impl->playStream) AAudioStream_requestStop(m_impl->playStream);
}
...
void FluxAudio::shutdown() { closePlayback(); closeRecord(); }

// ── Recording ─────────────────────────────────────────────────────────────────

bool FluxAudio::openRecord(int sampleRate, int channels) {
    if (m_impl->recStream) closeRecord();
...
bool FluxAudio::startRecord(RecordCallback cb) {
    ...
    if (!m_impl->recStream && !openRecord()) return false;
...
void FluxAudio::stopRecord() {
    m_impl->recording = false;
    if (m_impl->recStream) AAudioStream_requestStop(m_impl->recStream);
}
