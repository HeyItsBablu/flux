...
//
// Exercises all four overlay widget types:
//   1. Tooltip       — hover over any labeled button
//   2. Dropdown      — select input with scrollable list
//   3. ContextMenu   — right-click any card
//   4. Dialog        — modal with nested input
//
// Usage:
//   app.build([&](){
//       return FluxApp("Overlay Test", BuildComponent<OverlayTestApp>(), AppTheme::light());
//   });
// =============================================================================
...
class OverlayTestApp : public Component {
    ...
    State<std::string> log;                 // event log shown in the panel
    State<std::string> dialogInput;         // text field inside the dialog
    State<std::string> dialogResult;        // last confirmed value
    State<std::string> contextTarget;       // which card was right-clicked

    // Dialog pointer — kept so buttons inside can call close()
...
public:
    ...
    WidgetPtr build() override {
        ...
        auto chip = [&](const std::string& label, COLORREF bg, COLORREF fg) -> WidgetPtr {
            return Container(Text(label)->setFontSize(10)->setTextColor(fg))
                ->setBackgroundColor(bg)->setBorderRadius(12)
                ->setPaddingAll(10, 3, 10, 3);
        };

        // ─────────────────────────────────────────────────────────────────────
        // SECTION 1 — TOOLTIP
        // Four buttons, each with a different tooltip position / style.
        // ─────────────────────────────────────────────────────────────────────
        ...
        auto makeTooltipBtn = [&](
                ...
                TooltipPosition    pos,
                COLORREF           bg,
                COLORREF           tipBg) -> WidgetPtr
        {
            auto btn = Button(label, [this, label]() {
                    appendLog("Clicked: " + label);
                })
                ->setBackgroundColor(bg)
                ->setHoverBackgroundColor(RGB(
                    min(255, GetRValue(bg) + 15),
                    min(255, GetGValue(bg) + 15),
                    min(255, GetBValue(bg) + 15)))
        ...
        auto tooltipSection = Column(
            ...
            SizedBox(0, 10),
            Row(
                makeTooltipBtn("Above (default)",
                    "Tooltip appears ABOVE the button.\nDefault position.",
                    TooltipPosition::Above,
                    kAccent, RGB(30,30,50)),

                makeTooltipBtn("Below",
                    "Tooltip appears BELOW the button.",
                    TooltipPosition::Below,
                    RGB(100, 60, 200), RGB(50,20,80)),

                makeTooltipBtn("Auto",
                    "Auto-positions based on available space.\nWill prefer above if room exists.",
                    TooltipPosition::Auto,
                    kGreen, RGB(10,50,30)),

                makeTooltipBtn("Long tip",
                    "This tooltip has a longer description that will word-wrap because setTooltipMaxWidth(160) is set.",
                    TooltipPosition::Above,
                    RGB(180,80,30), RGB(60,20,0))
                 
                // Use a separate path for the custom-width one:
            )->setSpacing(12)->setCrossAxisAlignment(CrossAxisAlignment::Center)
        )->setSpacing(0);

        // Rebuild last button with custom max width properly
        auto longTipBtn = Button("Long tip (160px)",
                [this](){ appendLog("Clicked: Long tip"); })
            ->setBackgroundColor(RGB(180,80,30))
            ->setHoverBackgroundColor(RGB(200,100,50))
            ->setTextColor(RGB(255,255,255))
        ...
        auto tooltipRow = Row(
            ...
            Tooltip(longTipBtn, "This tooltip has a longer description that word-wraps because setTooltipMaxWidth is 160px wide.")
                ->setPosition(TooltipPosition::Above)
                ->setTooltipBackground(RGB(60,20,0))
                ->setTooltipTextColor(RGB(255,220,180))
                ->setTooltipMaxWidth(160)
        )->setSpacing(12)->setCrossAxisAlignment(CrossAxisAlignment::Center);

        // ─────────────────────────────────────────────────────────────────────
        // SECTION 2 — DROPDOWN
        // Two dropdowns: one bound by index, one by string value.
        // ─────────────────────────────────────────────────────────────────────
        ...
        auto dropdownSection = Column(
            sectionLabel("2 · DROPDOWN  —  click to open, ↑↓ to navigate, Enter to confirm"),
            SizedBox(0, 10),
            Row(
                Column(
        ...
        // Three cards, each with its own right-click context menu.
        // ─────────────────────────────────────────────────────────────────────
        ...
        auto makeCard = [&](
                ...
                const std::vector<ContextMenuItem>& items) -> WidgetPtr
        {
                ...
                Column(
                    Row(
                        Container(nullptr)
                            ...
                            ->setBackgroundColor(accentColor)->setBorderRadius(2),
                        SizedBox(10, 0),
                        Column(
                            Text(cardName)->setFontSize(13)->setFontWeight(FontWeight::Bold)->setTextColor(kText),
            ...
            )->setWidth(195)->setBackgroundColor(kCard)
        ...
        auto cardSection = Column(
            ...
            Row(
                makeCard("Document.pdf", "Last edited 2 min ago", RGB(79,109,245), {
                    ...
                    ContextMenuItem("✏️ Rename",  [this](){ appendLog("ContextMenu → Rename Document.pdf"); }),
                ...
                makeCard("Image.png", "3.2 MB · 1920×1080", RGB(40,170,100), {
                    ContextMenuItem("👁 Preview",   [this](){ appendLog("ContextMenu → Preview Image.png"); }),
                    ContextMenuItem("✂️ Crop",       [this](){ appendLog("ContextMenu → Crop Image.png"); }),
                    ContextMenuItem("🔄 Rotate 90°",[this](){ appendLog("ContextMenu → Rotate Image.png"); }),
                    ContextMenuItem::Separator(),
                    ContextMenuItem("⬇️ Download",   [this](){ appendLog("ContextMenu → Download Image.png"); }),
                ...
                makeCard("Archive.zip", "Locked · 14 files", RGB(200,120,30), {
                    ...
                    ContextMenuItem("🗑 Delete",         [this, flag=false]() mutable {
                        appendLog("ContextMenu → Delete Archive.zip");
                    }),
                    ContextMenuItem::Separator(),
                    // Disabled item example
                    ContextMenuItem("⬆️ Upload (no permission)", nullptr, false),
                })
        ...
        // ─────────────────────────────────────────────────────────────────────
        // SECTION 4 — DIALOG
        // Three dialog variants: basic confirm, with input, non-dismissible.
        // ─────────────────────────────────────────────────────────────────────
        ...
        dlgA->setContent(
            Column(
                Text("Confirm Action")->setFontSize(16)->setFontWeight(FontWeight::Bold)->setTextColor(kText),
                SizedBox(0, 8),
                ...
                Row(
                    Button("Cancel", [dlgA](){
                        dlgA->close();
                    })->setBackgroundColor(RGB(230,232,240))
                     ->setTextColor(kText)->setBorderRadius(6)->setPadding(12),

                    Button("Confirm", [this, dlgA](){
                        appendLog("Dialog A → Confirmed");
                        dlgA->close();
                    })->setBackgroundColor(kAccent)
                     ->setHoverBackgroundColor(kAccentHv)
        ...
        // ── Dialog B: dialog with a TextInput ────────────────────────────────
        auto dlgB = Dialog();
        dlgB->setSize(400, 200)
             ->setCloseOnClickOutside(false)   // must use buttons
             ->setOverlayAlpha(160)
             ->setOnClose([this](){ appendLog("Dialog B closed"); });

        dlgB->setContent(
            Column(
                Text("Enter your name")->setFontSize(15)->setFontWeight(FontWeight::Bold)->setTextColor(kText),
                SizedBox(0, 10),
                ...
                Row(
                    ...
                    Button("Submit", [this, dlgB](){
                        ...
                        dialogInput.set("");
                        dlgB->close();
                    })->setBackgroundColor(kGreen)
                     ->setHoverBackgroundColor(RGB(30,150,80))
                     ->setTextColor(RGB(255,255,255))->setBorderRadius(6)->setPadding(12)
                )->setSpacing(8)->setCrossAxisAlignment(CrossAxisAlignment::Center)
                 ->setMainAxisAlignment(MainAxisAlignment::End)
            )->setSpacing(0)
        ...
        // ── Dialog C: danger confirmation, low alpha overlay ─────────────────
        auto dlgC = Dialog();
        ...
        dlgC->setContent(
            Column(
                Text("🗑  Delete everything?")->setFontSize(15)->setFontWeight(FontWeight::Bold)->setTextColor(kDanger),
                SizedBox(0, 8),
                Text("This action is permanent and irreversible.")->setFontSize(12)->setTextColor(kMuted),
                SizedBox(0, 18),
                Row(
                    Button("Keep it", [dlgC](){
                        dlgC->close();
                    ...
                    Button("Delete all", [this, dlgC](){
                        ...
                        dlgC->close();
                    })->setBackgroundColor(kDanger)
                     ->setHoverBackgroundColor(RGB(180,30,30))
                     ->setTextColor(RGB(255,255,255))->setBorderRadius(6)->setPadding(12)
                )->setSpacing(8)->setCrossAxisAlignment(CrossAxisAlignment::Center)
                 ->setMainAxisAlignment(MainAxisAlignment::End)
            )->setSpacing(0)
        ...
        auto mkDialogBtn = [&](const std::string& label, COLORREF bg, std::function<void()> fn) {
        ...
        auto dialogSection = Column(
            ...
            Row(
                Column(
                    mkDialogBtn("Simple Confirm", kAccent, [dlgA](){ dlgA->open(); }),
                    SizedBox(0, 4),
                    Text("closeOnClickOutside = true\noverlayAlpha = 140")
                        ->setFontSize(9)->setTextColor(kMuted)
                )->setSpacing(0),

                Column(
                    mkDialogBtn("With TextInput", kGreen, [dlgB](){ dlgB->open(); }),
                    SizedBox(0, 4),
                    Row(
                ...
                Column(
                    mkDialogBtn("Danger / Low Overlay", kDanger, [dlgC](){ dlgC->open(); }),
                    SizedBox(0, 4),
                    Text("closeOnClickOutside = true\noverlayAlpha = 80 (subtle)")
                        ->setFontSize(9)->setTextColor(kMuted)
                )->setSpacing(0),

                // The dialogs themselves live here in the tree so the
                // scaffold can wire them up — they render via overlay, not inline.
                dlgA,
                dlgB,
                dlgC
        ...
        auto logPanel = Container(
            Column(
                Row(
                    Text("EVENT LOG")->setFontSize(9)->setFontWeight(FontWeight::Bold)
                    ...
                    Expanded(
                        Button("Clear", [this](){ log.set("Log cleared."); })
                            ...
                            ->setBorderRadius(4)->setPaddingAll(8,2,8,2)
                    )
                )->setSpacing(8)->setCrossAxisAlignment(CrossAxisAlignment::Center),
                SizedBox(0, 6),
        ...
        )->setBackgroundColor(kLogBg)
         ->setBorderRadius(8)->setPaddingAll(14, 12, 14, 12);
        ...
        return Scaffold(
            AppBar("Overlay Widget Test — Tooltip · Dropdown · ContextMenu · Dialog"),
            Container(
                Column(
                    tooltipRow,
                    Divider(),
...
// =============================================================================
// ENTRY POINT (uncomment to use standalone)
// =============================================================================
//
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int) {
    ...
    app.createWindow("Overlay Test", 1000, 780);
    return app.run();
}