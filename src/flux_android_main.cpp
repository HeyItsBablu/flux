...
static void canvasGL_registerFonts() {
    if (!s_canvasGL) return;
    struct { const char* name; const char* path; } fonts[] = {
...
static void canvasGL_create() {
    if (!s_canvasGL) s_canvasGL = new Canvas2DGL();
    if (!s_canvasGL->init()) {
...
static void canvasGL_reinit() {
    if (!s_canvasGL) { canvasGL_create(); return; }
    s_canvasGL->destroy();
    s_canvasGL->init();
    canvasGL_registerFonts();
}
...
static void canvasGL_destroyGLObjects() {
    if (s_canvasGL) s_canvasGL->destroy();
}
...
static void handle_cmd(android_app* app, int32_t cmd) {
    switch (cmd) {
        ...
        case APP_CMD_TERM_WINDOW:
            ...
            if (s_app) s_app->getPlatformWindow().destroySurface();
