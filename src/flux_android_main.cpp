...
static void extractCACert(android_app* app) {
    ...
    __android_log_print(ANDROID_LOG_INFO, "FluxHttp",
                        "extractCACert: wrote %zu bytes to %s",
                        len, s_cacertPath.c_str());
}
...
std::string FluxAndroid_getFilesDir() {
    ...
    jmethodID getAbsPath  = env->GetMethodID(fileCls, "getAbsolutePath",
                                             "()Ljava/lang/String;");
...
{
    JNIEnv* env = getJNIEnv();
    if (!env || !s_activity) return "";

    jobject activityObj = s_activity->clazz;
    jclass  activityCls = env->GetObjectClass(activityObj);

    jmethodID getResolver = env->GetMethodID(activityCls,
    ...
    __android_log_print(ANDROID_LOG_INFO, "FluxCamera",
                        "Saved to MediaStore: %s (%zu bytes)", filename.c_str(), dataSize);
    ...
    env->DeleteLocalRef(kDisplayName); env->DeleteLocalRef(vDisplayName);
    env->DeleteLocalRef(kMime);        env->DeleteLocalRef(vMime);
    env->DeleteLocalRef(kRelPath);     env->DeleteLocalRef(vRelPath);
    env->DeleteLocalRef(kPending);     env->DeleteLocalRef(kPending2);
    env->DeleteLocalRef(vOne);         env->DeleteLocalRef(vZero);
...
static void requestPermissionsFromNative() {
    ...
    jmethodID checkPerm = env->GetMethodID(activityCls,
                                           "checkSelfPermission", "(Ljava/lang/String;)I");
    jmethodID reqPerms  = env->GetMethodID(activityCls,
    ...
    jobjectArray permsArr = env->NewObjectArray(
            (jsize)needed.size(), strCls, nullptr);
    ...
    switch (cmd) {
        case APP_CMD_INIT_WINDOW: {
            ...
            if (!s_cacertPath.empty()) {
                ...
                __android_log_print(ANDROID_LOG_INFO, "FluxHttp",
                                    "CA bundle set to: %s", s_cacertPath.c_str());
            }
