...
class VectorApp : public Component {
  ...
  static constexpr double kZoomMin = 6.25, kZoomMax = 800.0;
...
public:
  ...
  WidgetPtr build() override {
    ...
    canvas->onViewportChanged = [this](float z) { syncZoom(z); };

    // ── Wire state → surface ───────────────────────────────────────────────
    activeTool .listen([this](VTool t)          { if (surface_) surface_->setTool(t); });
    ...
    auto menuItem = [&](const std::string &label) -> WidgetPtr {
      return Container(
          Text(label)->setFontSize(11)->setTextColor(kText))
            ...
            Button("Clear",
                   [this] {
                     if (surface_) {
                       ...
                       for (auto &s : ids) surface_->removeShape(s.id);
                       refreshUndoRedo();
                       if (canvasPtr_) canvasPtr_->redraw();
                     }
                   })
          ...
          GestureDetector(
              Container(
                  Center(
                      Text(icon)
                          ->setFontSize(15)
                          ->setTextColor(activeTool, [tv](const VTool &at) {
        ...
        Column(
            Row(Text("Weight")
                ...
                Text(strokeWidth,
                     [](double v) {
                       char buf[16];
                       _snprintf_s(buf, sizeof(buf), _TRUNCATE, "%.2f pt", v);
    ...
    // =========================================================================
    // §G  HINTS BAR  (between control bar and canvas — like Illustrator's
    //                 "info bar" just above the rulers)
    // =========================================================================
    // (omitted for cleaner Illustrator look — hints moved to tooltip + status)
    ...
    //  appBar
    //  controlBar
    //  ┌─ toolStrip ─┬─ canvasWithMenu ─┬─ rightPanel ─┐
    //  └─────────────┴──────────────────┴──────────────┘
    //  statusBar
    ...
    auto centerRow =
        Row(toolStrip,
            Container(canvasWithMenu)
                ->setBackgroundColor(kBg),  // grey pasteboard bg visible at edges
            rightPanel)
            ->setSpacing(0);
