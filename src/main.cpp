...
static std::vector<float> star5Outline(float cx, float cy, float ro, float ri,
  ...
  for (int i = 0; i < 10; i++) {
    ...
    total = appendThickLine(cx + cosf(a0) * r0, cy + sinf(a0) * r0,
                            cx + cosf(a1) * r1, cy + sinf(a1) * r1, half,
                            buf.data(), total);
  }
  buf.resize(total * 2);
  return buf;
...
static std::vector<float> ellipseOutline(float cx, float cy, float rx, float ry,
  ...
  for (int i = 0; i < segs; i++) {
    ...
    total = appendThickLine(cx + cosf(a0) * rx, cy + sinf(a0) * ry,
                            cx + cosf(a1) * rx, cy + sinf(a1) * ry, half,
                            buf.data(), total);
  }
  buf.resize(total * 2);
  return buf;
...
static std::vector<float> heartFan(float cx, float cy, float rx, float ry) {
  ...
  for (int i = 0; i < segs; i++) {
    ...
    float hy =
        -ry *
        (13.f * cosf(t) - 5.f * cosf(2 * t) - 2.f * cosf(3 * t) - cosf(4 * t)) /
        16.f;
...
private:
  ...
  void selectMouseUp(float x, float y) {
    if (moving_) {
      ...
      selY1_ = y;
      hasSelection_ =
          (fabsf(selX1_ - selX0_) > 2.f && fabsf(selY1_ - selY0_) > 2.f);
      if (selX0_ > selX1_)
        std::swap(selX0_, selX1_);
      if (selY0_ > selY1_)
        std::swap(selY0_, selY1_);
      scratchClear();
      if (hasSelection_)
        drawDots(selX0_, selY0_, selX1_, selY1_);
      if (onRedrawNeeded)
  ...
  void blitPixelsToScratch(float x0, float y0) {
    ...
    int dstW = min(grabW_ - srcOffX, cw - dstLX),
        dstH = min(grabH_ - srcOffY, ch - dstBY);
    if (dstW <= 0 || dstH <= 0)
      return;
  ...
  void commitPixels(float x0, float y0) {
    ...
    for (int r = 0; r < dstH; r++)
      for (int c = 0; c < dstW; c++) {
        const uint8_t *s =
            pixelBuf_.data() + ((srcOffY + r) * grabW_ + srcOffX + c) * 4;
  ...
  void drawDots(float x0, float y0, float x1, float y1) {
    float lx = min(x0, x1), rx = max(x0, x1), by = min(y0, y1),
          ty = max(y0, y1);
  ...
  void drawShapeInto(GLuint fbo, float x0, float y0, float x1, float y1) {
    ...
    case Tool::Rect: {
      float lx = min(x0, x1), bby = min(y0, y1), rxx = max(x0, x1),
            ty = max(y0, y1);
      if (filled)
        rectFilled(lx, bby, rxx, ty);
      else
        rectOutline(lx, bby, rxx, ty);
      break;
    }
    case Tool::Ellipse: {
      if (filled) {
    ...
    case Tool::Triangle: {
      ...
      if (filled) {
        ...
        for (int i = 0; i < 3; i++) {
          int j = (i + 1) % 3;
          total = appendThickLine(pts[i].first, pts[i].second, pts[j].first,
                                  pts[j].second, half, buf, total);
        }
        drawVertsToFBO(fbo, buf, total, GL_TRIANGLES, r, g, b, a);
      }
      break;
    ...
    case Tool::Pentagon: {
      if (filled) {
        auto v = nGonFilled(cx, cy, rx, ry, 5, -1.5708f);
        emitFan(v);
      } else {
        auto v = nGonOutline(cx, cy, rx, ry, 5, -1.5708f, half);
        emitOutline(v);
      }
      break;
    }
    case Tool::Hexagon: {
      if (filled) {
        auto v = nGonFilled(cx, cy, rx, ry, 6, 0);
        emitFan(v);
      } else {
        auto v = nGonOutline(cx, cy, rx, ry, 6, 0, half);
        emitOutline(v);
      }
      break;
    }
    ...
    case Tool::Arrow: {
      ...
      if (!v.empty())
        drawVertsToFBO(fbo, v.data(), (int)(v.size() / 2), GL_TRIANGLES, r, g,
                       b, a);
      break;
    }
    ...
    case Tool::RoundRect: {
      ...
      if (filled) {
        ...
        for (int i = 0; i < n; i++) {
          int j = (i + 1) % n;
          total = appendThickLine(pts[i].first, pts[i].second, pts[j].first,
                                  pts[j].second, half, bufv.data(), total);
        }
...
public:
  PaintApp()
      : activeColor("#cba6f7", context), activeSize(4.0, context),
        ...
        filledShapes(false, context), canUndo(false, context),
        canRedo(false, context), zoomLevel(100.0, context),
        colorHistory({}, context), statusMsg("", context),
        fontSize(20, context), layerDescs({}, context) {}
  ...
  WidgetPtr build() override {
    ...
    surface_->onLayersChanged = [this](const LayerState &ls) {
      ...
      for (auto &ld : ordered)
        activeNames.insert(ld.name);

      for (auto it = layerRefByName_.begin(); it != layerRefByName_.end();) {
        if (!activeNames.count(it->first))
          it = layerRefByName_.erase(it);
        else
          ++it;
      }
      ...
      if (structuralChange || int(layerDescs.get().size()) != count ||
          orderChanged)
    ...
    filledShapes.listen([this](bool f) {
      if (surface_)
        surface_->setFilled(f);
    });
    ...
    auto makeToolBtn = [&](Tool tv, const std::string &icon,
      ...
      return GestureDetector(
                 Container(
                     Center(Column(Text(icon)->setFontSize(16)->setTextColor(
                                       activeTool,
                                       [tv](const Tool &at) {
    ...
    auto drawRow = std::make_shared<RowWidget>();
    ...
    auto drawRow2 = std::make_shared<RowWidget>();
    drawRow2->setSpacing(4);
    drawRow2->addChild(makeToolBtn(Tool::Text, "𝐓", "Text"));

    auto drawSection = cardWrap(Column(sectionLabel("DRAW"), SizedBox(0, 8),
                                       drawRow, SizedBox(0, 4), drawRow2)
                                    ->setSpacing(0));
    ...
    const std::vector<SD> basicShapes = {{Tool::Line, "╱", "Line"},
                                         {Tool::Rect, "▭", "Rect"},
                                         {Tool::RoundRect, "▢", "RndRect"},
                                         {Tool::Ellipse, "⬭", "Ellipse"}};
    const std::vector<SD> polyShapes = {{Tool::Triangle, "△", "Triangle"},
                                        {Tool::RightTriangle, "◺", "R.Tri"},
                                        {Tool::Diamond, "◇", "Diamond"},
                                        {Tool::Pentagon, "⬠", "Pentagon"}};
    const std::vector<SD> starShapes = {{Tool::Hexagon, "⬡", "Hexagon"},
                                        {Tool::Star5, "★", "Star 5"},
                                        {Tool::Star6, "✡", "Star 6"},
                                        {Tool::Cross, "✛", "Cross"}};
    ...
    auto makeShapeRow =
        [&](const std::vector<SD> &shapes) -> std::shared_ptr<RowWidget> {
      auto row = std::make_shared<RowWidget>();
      row->setSpacing(4);
      for (auto &s : shapes)
        row->addChild(makeToolBtn(s.t, s.icon, s.label));
      return row;
    };
    ...
    auto fillToggle =
        GestureDetector(
            Container(Row(Container(nullptr)
                              ...
                              ->setHeight(14)
                              ->setBorderRadius(3)
                              ->setBackgroundColor(filledShapes,
                                                   [](const bool &f) {
                                                     return f ? RGB(174, 129,
                                                                    255)
                                                              : RGB(17, 17, 27);
                                                   })
                              ->setBorderWidth(1)
    ...
    auto shapeSection =
        cardWrap(Column(sectionLabel("SHAPES"), SizedBox(0, 8),
                        ...
                        makeShapeRow(extraShapes), SizedBox(0, 8), fillToggle)
                     ->setSpacing(0));

    // ── §B2  FONT SIZE section ─────────────────────────────────────────────
    auto fontSizeSection = cardWrap(
        Row(sectionLabel("FONT SIZE"), SizedBox(6, 0),
            Row(GestureDetector(
                    Container(Text("–")->setFontSize(14)->setTextColor(
                                  RGB(180, 180, 200)))
                        ->setWidth(18)
                        ->setHeight(18)
                        ->setBorderRadius(4)
                        ...
                        ->setBorderColor(kBorder))
                    ->setOnTap(
                        [this]() { fontSize.set(max(8, fontSize.get() - 2)); }),
                SizedBox(4, 0),
                Container(Text(fontSize,
                               [](const int &sz) { return std::to_string(sz); })
                              ->setFontSize(10)
    ...
    auto paletteSection =
        cardWrap(Column(sectionLabel("PRESETS"), SizedBox(0, 8),
                ...
                ->Else([&] {
                  return Row(Text("MMB/Space: Pan")
                             ...
                             Text("Ctrl+Z/Y: Undo/Redo")
                                 ...
                                 ->setTextColor(RGB(75, 75, 95)),
                             SizedBox(10, 0),
                             Text("B E L R F S T: Tools  |  T=Text, click to "
                                  "place, type, Enter=commit, Esc=cancel")
                                 ->setFontSize(10)
                                 ->setTextColor(RGB(75, 75, 95)))
                      ->setSpacing(0);
                }))
    ...
    auto layerList =
        ListView(layerDescs)
            ->itemBuilder([this, kCard, kBorder, kAccent, kDim,
              ...
              return Container(Row(eye, SizedBox(5, 0), name, upBtn,
                                   SizedBox(2, 0), dnBtn, SizedBox(3, 0),
                                   delBtn)
                                   ->setSpacing(0)
                                   ->setCrossAxisAlignment(
                                       CrossAxisAlignment::Center))
