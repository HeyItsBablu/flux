...
void Painter::drawTextDecorationLine(int lineX, int lineY, int lineW,
    ...
    case TextDecorationStyle::Solid:
        drawHLine(lineX, decorY, lineW, dc, thickness); break;
    case TextDecorationStyle::Double:
        drawHLine(lineX, decorY,     lineW, dc, thickness);
        drawHLine(lineX, decorY + 2, lineW, dc, thickness); break;
    ...
    case TextDecorationStyle::Wavy:
        drawWavyLine(lineX, decorY, lineW, dc, 2); break;
    }
}
...
static int measureLineNVG(NVGcontext* vg, const std::string& utf8,
                           ...
                           int& outLineH) {
    if (!vg || len <= 0) { outLineH = 16; return 0; }
...
void Painter::measureRichText(const std::wstring& text,
    ...
    if (f && f->nvgHandle != -1) nvgFontFaceId(s_vg, f->nvgHandle);
...
void Painter::drawRichText(const std::wstring& text,
                            ...
                            FontCache& fontCache) {
    if (!s_vg || text.empty() || params.w <= 0 || params.h <= 0) return;
...
void Painter::drawRichTextA(const std::string& text,
    ...
    for (unsigned char c : text) wide += static_cast<wchar_t>(c);
