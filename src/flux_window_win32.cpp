...
bool PlatformWindow::create(const std::string &title, int width, int height,
    ...
    hwnd = CreateWindowEx(
        0, "FluxUI", title.c_str(),
        WS_OVERLAPPEDWINDOW | WS_VISIBLE,
        CW_USEDEFAULT, CW_USEDEFAULT,
        r.right - r.left, r.bottom - r.top,
        NULL, NULL, hInstance, userData);   // userData goes to WM_CREATE
...
LRESULT CALLBACK PlatformWindow::WindowProc(HWND hwnd, UINT uMsg,
    ...
    case WM_MOUSEWHEEL: {
        ...
        if (self->callbacks.onMouseWheel &&
            self->callbacks.onMouseWheel(delta))
            self->invalidate();
        return 0;
    }
    ...
    case WM_RBUTTONDOWN: {
        if (!self) return 0;
        int x = LOWORD(lParam), y = HIWORD(lParam);
        if (self->callbacks.onRightClick &&
            self->callbacks.onRightClick(x, y))
            self->invalidate();
        return 0;
    }
    ...
    case WM_MOUSEMOVE: {
        ...
        if (self->callbacks.onMouseMove &&
            self->callbacks.onMouseMove(x, y))
            self->invalidate();
        return 0;
    }
    ...
    case WM_CHAR: {
        ...
        if (self->callbacks.onChar &&
            self->callbacks.onChar(ch))
            self->invalidate();
        return 0;
    }
    ...
    case WM_KEYDOWN: {
        ...
        if (self->callbacks.onKeyDown &&
            self->callbacks.onKeyDown(keyCode))
            self->invalidate();
        return 0;
    }
