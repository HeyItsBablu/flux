...
static Win32CanvasState& win32State(CanvasWidget* w) {
    return s_win32State[w];
}
...
static bool tryUpgradeToMSAA(CanvasWidget* w, int winW, int winH, HGLRC bootstrapCtx) {
    auto& s = win32State(w);
    ...
    for (int samples : {8, 4, 2}) {
        ...
        if (!wglCPF(s.glDC, attribs, nullptr, 1, &fmt, &numFormats) || !numFormats) continue;
        wglMakeCurrent(nullptr, nullptr);
        wglDeleteContext(bootstrapCtx);
        ReleaseDC(s.glHwnd, s.glDC);
        DestroyWindow(s.glHwnd);
        s.glHwnd = CreateWindowExW(WS_EX_NOPARENTNOTIFY, L"FluxGLCanvas9", nullptr,
            WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS|WS_CLIPCHILDREN,
            0, 0, winW, winH, s.frameHwnd, nullptr, GetModuleHandle(nullptr), nullptr);
        assert(s.glHwnd);
...
static void tickAndRender(CanvasWidget* w) {
    auto& s = win32State(w);
    if (!s.glRC || !s.glDC || !w->canvasGL_) return;
    if (w->pendingSurface_) w->activatePendingSurface();
...
static void ensureWindows(CanvasWidget* w, GraphicsContext& ctx) {
    (void)ctx;
    auto& s = win32State(w);
    if (s.frameHwnd) return;

    s.parentHwnd = FluxUI::getCurrentInstance()->getWindow();
    if (!s.parentHwnd) return;
    registerClasses();

    s.frameHwnd = CreateWindowExW(WS_EX_NOPARENTNOTIFY, L"FluxGLFrame9", nullptr,
        WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS|WS_CLIPCHILDREN,
        w->x, w->y, w->width, w->height,
        s.parentHwnd, nullptr, GetModuleHandle(nullptr), nullptr);
    assert(s.frameHwnd);
    ...
    s.glHwnd = CreateWindowExW(WS_EX_NOPARENTNOTIFY, L"FluxGLCanvas9", nullptr,
        ...
        s.frameHwnd, nullptr, GetModuleHandle(nullptr), nullptr);
    assert(s.glHwnd);
    SetWindowLongPtrW(s.glHwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(w));
...
static void moveWindows(CanvasWidget* w) {
    ...
    if (w->width != s.lastGLW || w->height != s.lastGLH) {
        makeCurrent(w);
        s.lastGLW = w->width; s.lastGLH = w->height;
        w->updateViewportSize(w->width, w->height);
...
static void destroyGL(CanvasWidget* w) {
    ...
    if (s.glRC) {
        ...
        if (w->canvasGL_) { w->canvasGL_->destroy(); delete w->canvasGL_; w->canvasGL_ = nullptr; }

        if (w->sbProg_) { glDeleteProgram(w->sbProg_);          w->sbProg_ = 0; }
        if (w->sbVAO_)  { glDeleteVertexArrays(1, &w->sbVAO_);  w->sbVAO_  = 0; }
        if (w->sbVBO_)  { glDeleteBuffers(1, &w->sbVBO_);       w->sbVBO_  = 0; }

        wglMakeCurrent(nullptr, nullptr);
        wglDeleteContext(s.glRC); s.glRC = nullptr;
    }
    if (s.glHwnd && s.glDC) { ReleaseDC(s.glHwnd, s.glDC); s.glDC = nullptr; }
    if (s.glHwnd)    { DestroyWindow(s.glHwnd);    s.glHwnd    = nullptr; }
    if (s.frameHwnd) { DestroyWindow(s.frameHwnd); s.frameHwnd = nullptr; }
...
static LRESULT CALLBACK FrameProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    auto* w = reinterpret_cast<CanvasWidget*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
    switch (msg) {
...
static LRESULT CALLBACK GLProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_USER+1:
        if (w) { win32State(w).repaintPending = false; tickAndRender(w); }
        return 0;
    ...
    case WM_MBUTTONDOWN:
        if (!w || !w->viewportEnabled_) return 0;
        SetCapture(hwnd);
        w->beginPan(GET_X_LPARAM(lp), GET_Y_LPARAM(lp));
        return 0;
    case WM_MBUTTONUP:
        if (!w || !w->viewportEnabled_) return 0;
        ReleaseCapture(); w->panning_ = false; return 0;

    case WM_LBUTTONDOWN: {
        SetCapture(hwnd); SetFocus(hwnd);
        if (!w) return 0;
        int sx = GET_X_LPARAM(lp), sy = GET_Y_LPARAM(lp);
        bool hC = w->hBar_.onMouseDown(sx, sy,
            [w](float t){ w->applyHScrollFraction(t); });
        bool vC = !hC && w->vBar_.onMouseDown(sx, sy,
            [w](float t){ w->applyVScrollFraction(t); });
        if (!hC && !vC) {
    ...
    case WM_LBUTTONUP: {
        ...
        if (!hR && !vR) {
            if (w->panning_) { w->panning_ = false; }
            else if (w->activeSurface_) {
                auto [cx, cy] = w->vp_.screenToCanvas(float(sx), float(sy));
    ...
    case WM_RBUTTONDOWN: {
        ...
        if (w && w->activeSurface_) {
            auto [cx, cy] = w->vp_.screenToCanvas(
    ...
    case WM_MOUSEMOVE: {
        ...
        if (w->panning_) { w->continuePan(sx, sy); }
        else if (w->activeSurface_) {
            auto [cx, cy] = w->vp_.screenToCanvas(float(sx), float(sy));
    ...
    case WM_MOUSELEAVE:
        if (w) {
            ...
            w->hBar_.onMouseLeave(); w->vBar_.onMouseLeave();
            scheduleRepaint(w);
        }
        return 0;
    ...
    case WM_MOUSEWHEEL: {
        ...
        w->pokeScrollbars(); scheduleRepaint(w);
        forwardToParent(hwnd, msg, wp, lp); return 0;
    }
    ...
    case WM_KEYDOWN: {
        ...
        if (ctrl && w->viewportEnabled_) {
            if (wp == Key::OemPlus || wp == Key::NumpadPlus) {
                w->vp_.zoomIn();    w->pokeScrollbars(); scheduleRepaint(w); consumed=true;
            } else if (wp == Key::OemMinus || wp == Key::NumpadMinus) {
                w->vp_.zoomOut();   w->pokeScrollbars(); scheduleRepaint(w); consumed=true;
            } else if (wp == '0') {
                w->vp_.resetZoom(); w->pokeScrollbars(); scheduleRepaint(w); consumed=true;
            }
        }
    ...
    case WM_KEYUP:
        if (w && w->activeSurface_) w->activeSurface_->onKeyUp(int(wp));
        return 0;
...
std::shared_ptr<CanvasWidget> CanvasWidget::setScrollbarsEnabled(bool e) {
    ...
    if (s.glHwnd) { updateViewportSize(s.lastGLW, s.lastGLH); scheduleRepaint(this); }
    return ptr();
}
