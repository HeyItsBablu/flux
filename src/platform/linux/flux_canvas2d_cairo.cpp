...
void Canvas2D::arcTo(float x1, float y1, float x2, float y2, float radius)
{
    ...
    if (cross > 0)
        cairo_arc(cr, arcCx, arcCy, radius, a0, a1);
    else
        cairo_arc_negative(cr, arcCx, arcCy, radius, a0, a1);
    curX_ = (float)t1x;
    curY_ = (float)t1y;
}
...
{
    if (rgbaData.size() < (size_t)srcW * srcH * 4)
        return;
