...
struct MFInit {
    ...
    ~MFInit() { MFShutdown(); CoUninitialize(); }
};
...
static void decodeLoop(std::string path) {
    ...
    attrs->Release();
    if (FAILED(hr)) { s.state = FluxVideo::State::Error; return; }
    ...
    for (DWORD i = 0; i < 8; i++) {
        ...
        if (SUCCEEDED(reader->GetCurrentMediaType(i, &mt)) && mt) {
            ...
            if (major == MFMediaType_Video && videoStreamIdx == 0xFFFFFFFF) videoStreamIdx = i;
            else if (major == MFMediaType_Audio && audioStreamIdx == 0xFFFFFFFF) audioStreamIdx = i;
            mt->Release();
        }
    }
    ...
    // Allocate frame buffer
    {
        ...
        s.frameWidth  = w; s.frameHeight = h; s.frameStride = w * 3;
    ...
    if (s.readyCallback) s.readyCallback(s.videoWidth.load(), s.videoHeight.load());
...

    {
        ...
        if (s.stopDecode) { reader->Release(); return; }
...
void FluxVideo::seekToProgress(float p) {
    ...
    auto& s = impl();
    std::lock_guard<std::mutex> lk(s.cmdMutex);
    s.seekTargetHns = hns; s.seekPending = true; s.cmdCV.notify_one();
}
