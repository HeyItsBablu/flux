...
namespace
{
    ...
    int dispatch_doctor(const std::string &platform)
    {
...
#else
            std::fprintf(stderr, "flux: 'windows' doctor checks require running flux from a Windows host.\n");
            return 1;
#endif
        }
        ...
        if (platform == "macos")
        {
            ...
            return macos_doctor();
#else
            std::fprintf(stderr, "flux: 'macos' doctor checks require running flux from a macOS host.\n");
            return 1;
#endif
        }
        ...
        if (platform == "linux")
        {
            ...
            return linux_doctor();
#else
            std::fprintf(stderr, "flux: 'linux' doctor checks require running flux from a Linux host.\n");
            return 1;
#endif
        }
