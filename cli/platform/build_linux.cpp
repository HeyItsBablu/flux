...
bool pkg_config_exists(const std::string &lib) {
  return std::system(
             ("pkg-config --exists " + lib + " 2> /dev/null").c_str()) == 0;
}
...
std::optional<fs::path> find_project_root() {
  ...
  for (int i = 0; i < 16; ++i) {
    if (fs::exists(dir / "CMakeLists.txt") &&
        fs::exists(dir / "config" / "AppConfig.cmake")) {
      return dir;
    }
...
std::pair<int, fs::path> do_configure_and_build(bool release,
                                                const std::string &build_dir) {
  auto root = find_project_root();
  if (!root) {
    std::fprintf(stderr, "ERROR: could not locate project root "
                         "(no CMakeLists.txt / config/AppConfig.cmake found "
                         "above current directory).\n");
    return {1, {}};
  }

  std::string command = "cd \"" + root->string() +
                        "\" && "
                        "cmake -S . -B " +
                        build_dir +
                        " -G Ninja "
                        "-DCMAKE_BUILD_TYPE=" +
                        std::string(release ? "Release" : "Debug") +
                        " "
  ...
  std::printf("Building flux (%s, Linux)...\n\n",
...
int linux_doctor() {
  ...
  check("project root found", root.has_value(),
        root ? root->string()
             : "no CMakeLists.txt / config/AppConfig.cmake found above cwd");
...
{
    auto [rc, root] = do_configure_and_build(release, release ? "build/linux-release" : "build/linux");
    if (rc == 0)
        std::printf("\nBuild succeeded.\n");
    return rc;
}
...
{
    auto build_dir = release ? "build/linux-release" : "build/linux";
    auto [rc, root] = do_configure_and_build(release, build_dir);
    if (rc != 0)
        return rc;

    fs::path exe = root / build_dir / "linux" / "flux_app";
    if (!fs::exists(exe))
    {
...
// exposed for linux_installer.cpp, same role as windows_configure_and_build
std::pair<int, fs::path> linux_configure_and_build(bool release, const std::string &build_dir)
{
    return do_configure_and_build(release, build_dir);
}