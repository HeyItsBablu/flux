...
#include "installer/linux_common.hpp"
#include "installer/deb_installer.hpp"
...
namespace
{
  ...
  int release_appimage()
  {
    ...
    fs::path toolCacheDir = flux_tool_cache_dir();
    ...
    std::string deployCmd =
        ...
        appDir.string() + "\" "
                          "--executable \"" +
        (appDir / "usr" / "bin" / "flux_app").string() + "\" "
                                                         "--desktop-file \"" +
        (appDir / desktopFileName).string() + "\"";
    if (haveIcon)
...
{
  if (format == "appimage" || format.empty())
    return release_appimage();
  if (format == "deb")
    return linux_release_deb();

  std::fprintf(stderr, "flux: unknown --format '%s' for linux release (expected 'appimage' or 'deb')\n",
               format.c_str());
  return 1;
}