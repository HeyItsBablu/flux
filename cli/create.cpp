...
std::string slugify(const std::string& name) {
    std::string out;
    for (char c : name) out += std::isalnum(static_cast<unsigned char>(c)) ? static_cast<char>(std::tolower(c)) : '-';
    return out;
}
...
void strip_dev_only(const fs::path& root) {
    for (const char* rel : {"examples", "screenshots", "docs", ".git"}) {
        std::error_code ec;
        fs::remove_all(root / rel, ec);
    }
    for (const char* rel : {"CHANGELOG.md", "todo.txt"}) {
        std::error_code ec;
