...
public:
    std::function<void(bool, const std::string &)> onExportDone; // (ok, message)
    ...
    void setHighlights (float v) { highlights_ .store(v, std::memory_order_relaxed); }
    ...
    void setSaturation (float v) { saturation_ .store(v, std::memory_order_relaxed); }
    void setVibrance   (float v) { vibrance_   .store(v, std::memory_order_relaxed); }
    void setTemperature(float v) { temperature_.store(v, std::memory_order_relaxed); }
    ...
    void render(Canvas2D &ctx) override {
        ...
        if (pendingLoad_.load()) {
            ...
            if (img_) {
                imgW_ = img_->width;
                imgH_ = img_->height;
                // Re-create the offscreen FBO at the new image size
        ...
        // ── Upload pending curve LUT ────────────────────────────────────────
        {
            ...
            if (curveLUTDirty_ && curveTex_) {
                ...
                glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 256, 4,
                                GL_RED, GL_UNSIGNED_BYTE,
                                pendingCurveLUT_.data());
        ...
        // ── Upload pending HSL LUTs ─────────────────────────────────────────
        {
            ...
            if (hslLUTDirty_) {
                if (hslHueTex_) {
                    ...
                    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 360, 1,
                                    GL_RED, GL_UNSIGNED_BYTE, pendingHSLHue_.data());
                }
                if (hslSatTex_) {
                    glBindTexture(GL_TEXTURE_2D, hslSatTex_);
                    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 360, 1,
                                    GL_RED, GL_UNSIGNED_BYTE, pendingHSLSat_.data());
                }
                if (hslLumTex_) {
                    glBindTexture(GL_TEXTURE_2D, hslLumTex_);
                    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 360, 1,
                                    GL_RED, GL_UNSIGNED_BYTE, pendingHSLLum_.data());
                }
                glBindTexture(GL_TEXTURE_2D, 0);
                hslLUTDirty_ = false;
            }
        ...
        // ── Export (runs on render thread where GL context is current) ──────
        ...
        renderDx_ = dx; renderDy_ = dy;
        renderDw_ = dw; renderDh_ = dh;
    ...
    void destroy() override {
        if (img_ && ctx_) { ctx_->freeImage(img_); img_ = nullptr; }
        ctx_ = nullptr;
        if (prog_)      { glDeleteProgram(prog_);            prog_ = 0; }
        if (vao_)       { glDeleteVertexArrays(1, &vao_);    vao_  = 0; }
        if (vbo_)       { glDeleteBuffers(1, &vbo_);         vbo_  = 0; }
        if (curveTex_)  { glDeleteTextures(1, &curveTex_);  curveTex_  = 0; }
        if (hslHueTex_) { glDeleteTextures(1, &hslHueTex_); hslHueTex_ = 0; }
        if (hslSatTex_) { glDeleteTextures(1, &hslSatTex_); hslSatTex_ = 0; }
        if (hslLumTex_) { glDeleteTextures(1, &hslLumTex_); hslLumTex_ = 0; }
        destroyExportFBO();
    }
...
private:
    ...
    int    imgW_  = 0, imgH_  = 0; // original image dimensions

    // Rendered viewport rect (for histogram readback)
    ...
    void buildCurveLUTTexture() {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, 256, 4, 0,
    ...
    void buildHSLLUTTextures() {
        auto make360 = [](GLuint &tex, uint8_t fill) {
            ...
            data.fill(fill);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, 360, 1, 0,
                         GL_RED, GL_UNSIGNED_BYTE, data.data());
            glBindTexture(GL_TEXTURE_2D, 0);
        };
    ...
    void bindAndDraw(GLuint prog, const float mvp[16], GLuint imgTex,
                     float iw, float ih) {
        glUseProgram(prog);
    ...
    void doExport(const std::string &path) {
        if (!img_ || !img_->texId || !exportFBO_ || imgW_ <= 0 || imgH_ <= 0) {
            if (onExportDone) onExportDone(false, "No image loaded.");
            return;
        }
        ...
        // Build an ortho MVP mapping [0,imgW]×[0,imgH] to NDC
        ...
        // Upload a quad covering the full image [0,iw]×[0,ih] with UV [0,1]
        float verts[] = {
            0.f,  0.f,  0.f, 0.f,
            iw,   0.f,  1.f, 0.f,
            iw,   ih,   1.f, 1.f,
        ...
        // Read pixels back (OpenGL origin = bottom-left → flip rows)
        ...
        int stride = imgW_ * 4;
        for (int row = 0; row < imgH_; ++row) {
            std::memcpy(flipped.data() + row * stride,
                        raw.data() + (imgH_ - 1 - row) * stride,
                        stride);
        }
        ...
        // Write file using stb_image_write
        bool ok = false;
        std::string ext;
        auto dotPos = path.rfind('.');
        if (dotPos != std::string::npos)
            ext = path.substr(dotPos); // includes the dot, e.g. ".png"
        ...
        if (ext == ".jpg" || ext == ".jpeg") {
            ...
            ok = stbi_write_bmp(path.c_str(), imgW_, imgH_, 4,
                                flipped.data()) != 0;
        } else {
            // Default: PNG
    ...
    void doHistogramReadback() {
        ...
        if (!cb) return;
        if (renderDw_ <= 0 || renderDh_ <= 0) return;

        int fbW = std::max(1, (int)ctx_->width());
        int fbH = std::max(1, (int)ctx_->height());

        int rx = std::max(0, (int)renderDx_);
        int ry = std::max(0, (int)renderDy_);
        int rw = std::min((int)renderDw_, fbW - rx);
        int rh = std::min((int)renderDh_, fbH - ry);
        if (rw <= 0 || rh <= 0) return;

        int strideX = std::max(1, rw / 256);
        int strideY = std::max(1, rh / 256);
        int readW   = (rw + strideX - 1) / strideX;
        int readH   = (rh + strideY - 1) / strideY;

        std::vector<uint8_t> pixels(readW * readH * 4);
        std::vector<uint8_t> rowBuf(rw * 4);
        int dstRow = 0;
        for (int srcRow = 0; srcRow < rh && dstRow < readH; srcRow += strideY) {
            int glY = fbH - (ry + srcRow) - 1;
            glReadPixels(rx, glY, rw, 1, GL_RGBA, GL_UNSIGNED_BYTE, rowBuf.data());
            uint8_t *dst = pixels.data() + dstRow * readW * 4;
            for (int col = 0, dstCol = 0; col < rw && dstCol < readW;
                 col += strideX, ++dstCol) {
        ...
        HistogramData hist = HistogramData::fromPixels(
            pixels.data(), readW * dstRow, 4);
        cb(std::move(hist));
    }
};
...
class LightRoomApp : public Widget {
    ...
    // Status bar colors
    static constexpr Color kColorOk  = Color::fromRGB(148, 226, 213); // green
    static constexpr Color kColorErr = Color::fromRGB(243, 139, 168); // red
...
public:
    WidgetPtr build() override {
        ...
        // ── Histogram refresh ────────────────────────────────────────────────
        surface->requestHistogramReadback([this](HistogramData hd) {
            histState.set(std::move(hd));
        });

        auto refreshHistogram = [ws, this]() {
            if (auto s = ws.lock())
                s->requestHistogramReadback([this](HistogramData hd) {
                    histState.set(std::move(hd));
                });
        };
        ...
        auto pushCurveLUTs = [ws, this, refreshHistogram](const ToneCurveData &d) {
            curveState.set(d);
            if (auto s = ws.lock())
                s->uploadCurveLUTs(d.rgb.buildLUT(), d.r.buildLUT(),
        ...
        auto sectionLabel = [](const char *title) -> WidgetPtr {
            return Text(title)->setFontSize(10)
                ->setTextColor(Color::fromRGB(110, 110, 140));
        };

        // ── Slider onChange helpers ───────────────────────────────────────────
#define SLIDER_CB(setter) \
    [ws, refreshHistogram](double v) { \
        if (auto s = ws.lock()) s->setter(float(v)); \
        refreshHistogram(); \
    }
                    ...
                    Column({
                        ...
                        Row({
                            ...
                            FilePicker()
                                ...
                                ->setShowPath(true)
                                ->bindPath(filePath)
                                ->setOnChanged([ws, refreshHistogram,
                            ...
                            // Export — Save dialog, fires requestExport
                            FilePicker("💾 Export")
                                ...
                                ->setShowPath(false)
                                ->setOnChanged([ws, this](const std::string &path) {
                            ...
                            Container(ListView({
                                ...
                                Histogram(258, 90)
                                    ...
                                    ->setOnZoneClicked([this](float pos) {
                                        sExposure.set(std::max(-5.0,
                                            std::min(5.0, (pos - 0.5) * 4.0)));
                                    }),

                                // LIGHT
                                sectionLabel("LIGHT"),
                                sliderCol("Exposure",   -3.0, 3.0, 0.1,  sExposure,
                                    SLIDER_CB(setExposure)),
                                sliderCol("Contrast",   -1.0, 1.0, 0.05, sContrast,
                                    SLIDER_CB(setContrast)),
                                sliderCol("Highlights", -1.0, 1.0, 0.05, sHighlights,
                                    SLIDER_CB(setHighlights)),
                                sliderCol("Shadows",    -1.0, 1.0, 0.05, sShadows,
                                    SLIDER_CB(setShadows)),
                                sliderCol("Whites",     -1.0, 1.0, 0.05, sWhites,
                                    SLIDER_CB(setWhites)),
                                sliderCol("Blacks",     -1.0, 1.0, 0.05, sBlacks,
                                    SLIDER_CB(setBlacks)),

                                // COLOR
                                sectionLabel("COLOR"),
                                sliderCol("Temperature", -1.0, 1.0, 0.05, sTemperature,
                                    SLIDER_CB(setTemperature)),
                                sliderCol("Tint",       -1.0, 1.0, 0.05, sTint,
                                    SLIDER_CB(setTint)),
                                sliderCol("Vibrance",   -1.0, 1.0, 0.05, sVibrance,
                                    SLIDER_CB(setVibrance)),
                                sliderCol("Saturation", -1.0, 1.0, 0.05, sSaturation,
                                    SLIDER_CB(setSaturation)),

                                // DETAIL
                                sectionLabel("DETAIL"),
                                sliderCol("Sharpness",   0.0, 1.0, 0.05, sSharpness,
