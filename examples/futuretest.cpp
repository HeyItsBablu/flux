                ...
                Column({
                    ...
                    Card(FetchBuilder(
                        ...
                        [](const AsyncSnapshot<std::string>& snap) -> WidgetPtr {
                            ...
                            std::string preview = snap.data.substr(0, 120) + "…";
                            return Column({
                                ...
                                Text(std::to_string(snap.data.size()) + " bytes total")
                                    ->setFontSize(11)
                                    ->setTextColor(Color::fromRGB(140, 140, 140)),
                            })->setSpacing(6);
                        }
                    ...
                    Card(JsonBuilder(
                        ...
                        [](const AsyncSnapshot<JsonValue>& snap) -> WidgetPtr {
                            ...
                            std::string city    = j.has("address")
                                ? j["address"]["city"].getString() : "—";
                            std::string company = j.has("company")
                    ...
                    Card(TypedJsonBuilder<Post>(
                        ...
                        [](const AsyncSnapshot<Post>& snap) -> WidgetPtr {
                            ...
                            return Column({
                                Row({
                                    Text("Post #" + std::to_string(p.id))
                                        ->setFontWeight(FontWeight::Bold),
                                    Text("  by user " + std::to_string(p.userId))
                                        ->setFontSize(12)
                                        ->setTextColor(Color::fromRGB(120,120,120)),
                                }),
                                Text(p.title)
                    ...
                    Card(Column({
                        Text("Cover photo (picsum.photos):")
                            ...
                            ->setTextColor(Color::fromRGB(100, 100, 100)),
                        SizedBox(0, 8),
                        NetworkImage("https://picsum.photos/seed/fluxui/600/200")
