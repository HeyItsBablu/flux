...
private:
    ...
    //
    // THREADING MODEL:
    //   set() may be called from any thread (message thread, render thread,
    //   background threads).  Widget fields (text, width, height, etc.) are
    //   only safe to mutate on the render thread.
    //
    //   Observer text updates and property-binding appliers are therefore
    //   posted to the render thread via FluxUI::postToRenderThread().
    //   When ui is null (unit-test / headless use) they run inline.

    void notifyObserversLocked()
    {
        // Build the text string once, before posting.
        std::string newText = valueToString(value);
        T snapVal = value; // copy for lambda capture

        // ── Observer (Text widget) updates ───────────────────────────────
        observers.erase(
        ...
        for (auto &weakWidget : observers)
        {
            ...
            {
                FluxUI *uiPtr = ui;
                ui->postToRenderThread(
                    [widget, newText, uiPtr]()
                    {
                        widget->text = newText;
                        uiPtr->updateWidget(widget.get());
                    });
            }
            else
            {
        ...
        for (auto &binding : propertyBindings)
        {
            ...
            {
                FluxUI *uiPtr = ui;
                auto applier = binding.applier;
                bool needsLayout = binding.needsLayout;

                ui->postToRenderThread(
                    [widget, applier, snapVal, needsLayout, uiPtr]()
                    {
                        applier(widget.get(), snapVal);
                        if (needsLayout)
                            uiPtr->partialRebuild(widget.get());
                        else
                            uiPtr->invalidateWidget(widget.get());
                    });
            }
            else
            {
                // Headless / unit-test path — run inline.
                binding.applier(widget.get(), snapVal);
            }
        }
    }
...
public:
    ...
    // Guard: skip notification when value hasn't changed.
    ...
    void set(T newValue)
    {
        ...
        setAndNotify(std::move(newValue));
    }
                      ...
                      bool needsLayout = false)
    {
        ...
        // Run the initial sync inline — this is called during widget
        // construction, always on the same thread, so no posting needed.
    ...
    State &operator+=(const Rhs &rhs)
    {
        setAndNotify(get() + static_cast<T>(rhs));
        return *this;
    }
    ...
    {
        mutate([&](T &v)
               { v.insert(v.begin(), std::move(item)); });
    }
    ...
    {
        mutate([&](T &v)
               { std::sort(v.begin(), v.end(), std::forward<Cmp>(cmp)); });
    }
    ...
    {
        mutate([&](T &v)
               { if (index < v.size()) v[index] = std::move(item); });
    }
    ...
    {
        mutate([&](T &m)
               { m.insert_or_assign(std::move(key), std::move(val)); });
    }
...
template <typename T, typename Rhs>
bool operator==(const State<T> &s, const Rhs &r) { return s.get() == r; }
template <typename T, typename Rhs>
bool operator!=(const State<T> &s, const Rhs &r) { return s.get() != r; }
template <typename T, typename Rhs>
bool operator<(const State<T> &s, const Rhs &r) { return s.get() < r; }
