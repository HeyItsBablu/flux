...
public:
    ...
    virtual void renderPopupContent(HDC /*hdc*/, FontCache & /*fc*/) {}
                   ...
                   FontCache &fontCache)
    {
        ...
        if (!popupClass_) registerPopupClass_();
        ...
        if (!popupHwnd_) {
            ...
            SetWindowPos(popupHwnd_, HWND_TOPMOST,
                         screenX, screenY, w, h,
    ...
    {
        if (popupHwnd_) paintLayered_(fontCache);
    }
    ...
    // ----------------------------------------------------------------
    static void forwardMouseEvent(HWND popupHwnd, UINT msg,
                                  WPARAM wp, LPARAM lp)
    {
...
private:
    ...
    {
        if (dibDC_ && dibBmp_ && popupW_ == w && popupH_ == h) return;
        freeDIB_();
    ...
    {
        if (!popupHwnd_ || popupW_ <= 0 || popupH_ <= 0) return;
        ...
        renderPopupContent(dibDC_, fontCache);
        ...
        UpdateLayeredWindow(popupHwnd_, nullptr, nullptr,
                            &dstSz, dibDC_, &srcPt, 0, &bf, ULW_ALPHA);
    }
