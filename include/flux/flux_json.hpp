...
private:
    ...
    JsonValue parseString() {
        ...
        expect('"');
        JsonValue v; v.data = result; return v;
    }
    ...
    JsonValue parseNumber() {
        ...
        JsonValue v; v.data = d; return v;
    }

    JsonValue parseBool() {
        if (src_.substr(pos_, 4) == "true")  { pos_ += 4; JsonValue v; v.data = true;  return v; }
        if (src_.substr(pos_, 5) == "false") { pos_ += 5; JsonValue v; v.data = false; return v; }
        throw std::runtime_error("Invalid boolean literal");
    }

    JsonValue parseNull() {
        if (src_.substr(pos_, 4) == "null") { pos_ += 4; JsonValue v; return v; }
    ...
    JsonValue parseArray() {
        ...
        JsonArray arr;
        skipWs();
        if (peek() == ']') { consume(); JsonValue v; v.data = arr; return v; }
        while (true) {
        ...
        JsonValue v; v.data = std::move(arr); return v;
    }
    ...
    JsonValue parseObject() {
        ...
        JsonObject obj;
        skipWs();
        if (peek() == '}') { consume(); JsonValue v; v.data = obj; return v; }
        while (true) {
            skipWs();
        ...
        JsonValue v; v.data = std::move(obj); return v;
    }
};