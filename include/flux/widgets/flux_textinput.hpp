...
public:
    ...
    std::shared_ptr<TextInputWidget> setInputType(InputType t)
    {
        inputType = t;
    ...
    std::shared_ptr<TextInputWidget> setValidator(std::function<bool(const std::string &)> fn)
    {
    ...
    std::shared_ptr<TextInputWidget> setOnValidationChanged(std::function<void(bool)> fn)
    {
...
private:
    ...
    void revalidate()
    {
        bool newValid = validator ? validator(inputValue) : builtinValidate(inputValue);
        if (newValid != valid_)
        {
            valid_ = newValid;
    ...
    void _renderDom(IDomAdapter *adapter)
    {
        ...
        char colbuf[48];
        snprintf(colbuf, sizeof(colbuf), "1px solid rgb(%d,%d,%d)", border.r, border.g, border.b);
...
// One factory, InputType selected via ->setInputType(...) — consistent
// with the rest of the toolkit (Dropdown->setOptions, etc.) instead of a
// growing family of per-type factory functions.
//
//   TextInput("you@example.com")->setInputType(InputType::Email);
//   TextInput("Password")->setInputType(InputType::Password);
