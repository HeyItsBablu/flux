...
public:
    ...
    {
        videoPath = p; _sourceType = VideoSourceType::Path;
        _sourceUrl.clear(); _sourceMemory.clear();
        return self();
    }
    std::shared_ptr<VideoPlayerWidget> setUrl(const std::string &url)
    {
        _sourceUrl = url; _sourceType = VideoSourceType::Url;
        videoPath.clear(); _sourceMemory.clear();
        return self();
    }
    std::shared_ptr<VideoPlayerWidget> setMemory(const std::vector<uint8_t> &bytes)
    {
        _sourceMemory = bytes; _sourceType = VideoSourceType::Memory;
        videoPath.clear(); _sourceUrl.clear();
        return self();
    }
    ...
    VideoPlayerWidget()
    {
        ...
        FluxVideo::get().setOnReady([this](int w, int h) {
            _vidW          = w;
            _vidH          = h;
            _fboSizeDirty  = true;   // was _nvgImageDirty
            markNeedsPaint();
        });
#else
        FluxVideo::get().setOnFinished([this]() {
...
#ifdef __ANDROID__
        // ── CHANGED: removed nvgDeleteImage — _fboTex is drawn directly ───────
        // No NVGcontext needed; just release GL objects.
                       ...
                       FontCache & /*fontCache*/) override
    {
        ...
        if (!_opened) { _opened = true; _openVideoSource(); }
    }
    ...
    void render(GraphicsContext &ctx, FontCache &fontCache) override
    {
...
#ifdef __ANDROID__
        {
            ...
            // Rebuild FBO when video dimensions change
            // CHANGED: removed vg param — no NVGcontext involved
            if (_fboSizeDirty && _vidW > 0 && _vidH > 0) {
                _fboSizeDirty = false;
                _rebuildFBO(_vidW, _vidH);
            }

            // Blit new OES frame into _fboTex
            if (vid.updateFrame() && _fbo && _oesProgram) {
                _blitOESToFBO(vid.getTextureId());
                _progress = vid.getProgress();
            }

            // Draw _fboTex directly — no readback, no NVG image
            // CHANGED: vp.frame = _fboTex (GLuint == NativeImage on Android)
            if (_fboTex && _fboW > 0 && _fboH > 0) {
                ...
                vp.frame = _fboTex;       // GLuint → NativeImage (no cast needed)
                vp.srcW  = _fboW;
                vp.srcH  = _fboH;
                vp.dstX  = (int)dstX;
                vp.dstY  = (int)dstY;
...
#elif defined(__APPLE__) && TARGET_OS_OSX
        {
                ...
                if (frame.data && frame.width > 0 && frame.height > 0) {
                    ...
                    if (frame.stride == expectedStride) {
                        ...
                        for (int row = 0; row < frame.height; ++row) {
                            ...
                            src += frame.stride; dst += expectedStride;
                        }
                    }
            ...
            if (!_frameCache.empty() && _cachedSrcW > 0 && ctx.cgContext) {
                if (_cgImageDirty) { _cgImageDirty = false; _rebuildCGImage(); }
                if (_cgImage) {
                    Painter::VideoDrawParams vp;
...
#elif defined(_WIN32)
        {
            ...
            if (_d2dBitmapDirty && !_frameCache.empty() && _cachedSrcW > 0 && ctx.dc) {
                ...
                if (!_d2dBitmap ||
                    _d2dBitmapW != _cachedSrcW || _d2dBitmapH != _cachedSrcH) {
                    _d2dBitmap   = nullptr;
                ...
                if (_d2dBitmap) {
                    ...
                    _d2dBitmap->CopyFromMemory(&destRect,
                        _frameCache.data(), (UINT32)(_cachedSrcW * 4));
                }
            }
    ...
    bool handleMouseDown(int mx, int my) override
    {
        ...
        if (!_inRect(mx, my, _barRect)) {
            _barVisible = true; _resetBarTimer(); _togglePlayPause();
            markNeedsPaint(); return true;
        }
        if (_inRect(mx, my, _playBtnRect)) {
            _togglePlayPause(); markNeedsPaint(); return true;
        }
        ...
        _barVisible = true; _resetBarTimer(); markNeedsPaint(); return true;
    }
...
private:
...
#ifdef __ANDROID__
    // Video source dimensions (set by onReady callback)
    int    _vidW = 0, _vidH = 0;

    // FBO — OES external texture is blitted into _fboTex each frame.
    // _fboTex is then passed directly to Painter::drawVideo as NativeImage.
    // REMOVED: int _nvgImage = -1 (no longer needed)
    // REMOVED: std::vector<uint8_t> _fboPixels (glReadPixels roundtrip eliminated)
    GLuint _fboTex      = 0;
    GLuint _fbo         = 0;
    int    _fboW        = 0, _fboH = 0;
    ...
    // RENAMED from _nvgImageDirty — set when video resolution changes
    bool   _fboSizeDirty    = false;
    ...
    void _togglePlayPause()
    {
        ...
        if (_finished) {
            ...
            vid.seekToProgress(0.f); vid.play();
            _playing = true; _startProgressTimer(); return;
        }
    ...
    void _seekFromMouse(int mx)
    {
        ...
        if (_finished && t < 0.999f) {
            _finished = false; FluxVideo::get().play();
            _playing = true; _startProgressTimer();
        }
        markNeedsPaint();
    }
    ...
    // ── Letterbox (Win32, Linux, macOS, Web) ──────────────────────────────────
#if defined(_WIN32) || defined(__linux__) || \
    (defined(__APPLE__) && TARGET_OS_OSX)  || defined(__EMSCRIPTEN__)
    void _letterbox(int srcW, int srcH,
    ...
    void _rebuildCGImage()
    {
        ...
        if (!_cgColorSpace) _cgColorSpace = CGColorSpaceCreateDeviceRGB();

        CGDataProviderRef provider = CGDataProviderCreateWithData(
            nullptr, _frameCache.data(), _frameCache.size(), nullptr);
        if (!provider) return;

        _cgImage = CGImageCreate(
            (size_t)_cachedSrcW, (size_t)_cachedSrcH,
            8, 24, (size_t)(_cachedSrcW * 3), _cgColorSpace,
            (CGBitmapInfo)((uint32_t)kCGBitmapByteOrderDefault |
    ...
    void _initGLResources()
    {
        ...
        // OES external texture → FBO blit shader
        // (same shader as flux_oes_blit.cpp — kept local so the widget
        //  is self-contained and doesn't depend on FluxOESBlit being inited)
        ...
        glAttachShader(_oesProgram, vs); glAttachShader(_oesProgram, fs);
    ...
    // CHANGED: removed NVGcontext* vg param and all nvgXxx calls.
    // _fboTex is the render target — drawn directly by Painter::drawVideo.
    void _rebuildFBO(int w, int h)
    {
        // Release old GL objects
        if (_fbo)    { glDeleteFramebuffers(1, &_fbo);    _fbo    = 0; }
        if (_fboTex) { glDeleteTextures(1,    &_fboTex);  _fboTex = 0; }
        // REMOVED: nvgDeleteImage / nvgCreateImageRGBA / _fboPixels allocation

        _fboW = w; _fboH = h;
        if (w <= 0 || h <= 0) return;

        // Render-target texture (regular GL_TEXTURE_2D — sampled by FluxGL Painter)
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            ...
            glDeleteTextures(1, &_fboTex); _fboTex = 0;
            glDeleteFramebuffers(1, &_fbo); _fbo = 0;
            return;
        }

        __android_log_print(ANDROID_LOG_DEBUG, "FluxVideo",
                            "FBO %dx%d ready, tex=%u", w, h, _fboTex);
    }

    // CHANGED: removed glReadPixels + nvgUpdateImage — GPU→CPU→GPU roundtrip eliminated.
    // The OES frame is blitted straight into _fboTex and drawn from there.
    void _blitOESToFBO(GLuint oesTexId)
    {
        ...
        // REMOVED: glReadPixels + nvgUpdateImage — _fboTex is the final texture
    ...
    void _renderBar(GraphicsContext & /*ctx*/, FontCache &fontCache, Painter &p)
    {
        ...
        _playBtnRect = {cx, barY + (barHeight - btnSz) / 2, btnSz, btnSz};
        {
            ...
            std::wstring g(1, FluxIcons::glyph(
                _playing ? FluxIcons::Pause : FluxIcons::Play));
