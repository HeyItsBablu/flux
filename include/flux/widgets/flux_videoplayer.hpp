...
public:
    ...
    void render(GraphicsContext &ctx, FontCache &fontCache) override
    {
        ...
        // ── Rebuild FBO when video size changes ───────────────────────────────
        ...
        // ── Latch new frame: updateTexImage first, then blit OES → FBO ────────
        bool gotFrame = vid.updateFrame();
        if (gotFrame && _fbo && _oesProgram)
        {
        ...
        {
            _renderStatusOverlay(p, fontCache, "\xe2\x80\xa6 Loading\xe2\x80\xa6",
                                 colLoadingText);
            needsPaint = false;
            return;
        }
        ...
        // Black background
        p.fillRect(x, y, width, height, Color::fromRGB(0, 0, 0));

        // ── Draw video frame via NVG ──────────────────────────────────────────
        if (_nvgImage >= 0 && _fboW > 0 && _fboH > 0 && vg)
        {
            ...
            // NVG_IMAGE_NODELETE means NVG doesn't own the texture —
            // we manage lifetime ourselves
            NVGpaint imgPaint = nvgImagePattern(vg,
                                                dstX, dstY, dstW, dstH, 0.f, _nvgImage, 1.f);
...
private:
    ...
    int _vidW = 0;
    int _vidH = 0;
    // ── In VideoPlayerWidget private members, replace _nvgImage / _nvgImageDirty ──

    GLuint _fboTex = 0; // GL_TEXTURE_2D — the composited frame
    GLuint _fbo = 0;    // FBO we blit OES → _fboTex into
    int _fboW = 0;
    int _fboH = 0;
    int _nvgImage = -1; // NVG handle wrapping _fboTex

    // OES blit program (created once)
    GLuint _oesProgram = 0;
    GLuint _oesVAO = 0; // or just use raw attrib 0
    GLuint _oesVBO = 0;

    bool _glResourcesReady = false;
    bool _nvgImageDirty = false; // still used: resize trigger
    std::vector<uint8_t> _fboPixels;

    // ── Add this helper — call once from render() before first blit ───────────────
    ...
    void _initGLResources()
    {
        ...
        // Vertex + fragment shader that samples an OES texture and writes RGB
        ...
        // Fullscreen quad: pos(-1..1) + uv(0..1)
        // Note: OES textures from MediaCodec are often vertically flipped
        // depending on device. Flip UV.y here: uv.y = 1.0 - y
    ...
    // ── Rebuild FBO + NVG image when video dimensions change ─────────────────────
    ...
    void _rebuildFBO(NVGcontext *vg, int w, int h)
    {
        ...
        // FBO texture — used as render target for OES blit
        ...
        // Create NVG image from blank data — we'll update it every frame
        // NVG_IMAGE_NEAREST avoids blurring on pixel-exact blits
        _nvgImage = nvgCreateImageRGBA(vg, w, h, 0,
                                       _fboPixels.data());
    ...
    void _blitOESToFBO(GLuint oesTexId)
    {
        ...
        // ── Readback FBO → CPU → NVG image update ──────────────────────────
        // glReadPixels from the FBO we just rendered into
        if (_nvgImage >= 0 && !_fboPixels.empty())
        {
            glReadPixels(0, 0, _fboW, _fboH,
    ...
    void _renderBar(GraphicsContext &ctx, FontCache &fontCache, Painter &p)
    {
        ...
        // Timestamp — measure in LOGICAL pixels
        ...
        // Guard: if measureText returned physical pixels (NVG measures at physical
        // scale before nvgScale is applied), divide back down.
        // A timestamp string at font-size 12 should never exceed ~120 logical px;
        // if tw is larger than half the widget width something is wrong.
        float dpi = FluxAndroid_getDpiScale();
        if (tw > width / 2)
            tw = (int)(tw / dpi);

        p.drawText(toWideString(ts),
                   cx, barY, tw + 4, barHeight,
                   tf, colText,
                   DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        cx += tw + 10;

        // Seek track — remaining width minus right margin
        int trackW = x + width - 12 - cx;
        if (trackW < 20)
            trackW = 20;
        ...
        // Thumb circle — centered on fill end
        int thumbX = cx + fillW - 6;
        int thumbY = midY - 6;
        p.drawEllipse(thumbX, thumbY, 12, 12, colThumb, colThumb, 0);
    }
