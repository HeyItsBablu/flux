...
inline VideoPlayerWidgetPtr VideoPlayer()
{
    return std::make_shared<VideoPlayerWidget>();
}

/// Path or URL, auto-detected by scheme prefix.
///
///   VideoPlayer("video/sample.mp4")              -> local path
///   VideoPlayer("https://example.com/video.mp4") -> streamed URL
///
inline VideoPlayerWidgetPtr VideoPlayer(const std::string &pathOrUrl)
{
...
inline VideoPlayerWidgetPtr VideoPlayer(const std::vector<uint8_t> &bytes)
{
    return std::make_shared<VideoPlayerWidget>()->setMemory(bytes);
}

/// In-memory raw pointer + length.
inline VideoPlayerWidgetPtr VideoPlayer(const uint8_t *data, size_t len)
{
