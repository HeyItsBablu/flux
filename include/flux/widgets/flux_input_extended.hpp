...
public:
    ...
    bool handleFocus(bool focused) override {
        ...
        if (focused) {
            HWND hw = _hwnd();
            if (hw) SetTimer(hw, cursorTimerId_, 530, nullptr);
            cursorVisible_ = true;
            // FIX: clear any stale selection when gaining focus so the first
            // keypress is never treated as "replace selection".
            _clearSelection();
        } else {
            HWND hw = _hwnd();
            if (hw) KillTimer(hw, cursorTimerId_);
    ...
    bool handleMouseDown(int mx, int my) override {
        ...
        // V-scrollbar
        if (sbV_.onMouseDown(mx, my, x, y, width, height, hw)) {
            _repaint();
            return true;
        }
        // H-scrollbar
        if (sbH_.onMouseDown(mx, my, x, y, width, height, hw)) {
            _repaint();
            return true;
        }
        ...
        if (GetKeyState(VK_SHIFT) & 0x8000) {
            ...
            cursorLine_ = line; cursorCol_ = col;
        } else {
            _clearSelection();
            cursorLine_ = line; cursorCol_ = col;
            selAnchorLine_ = line; selAnchorCol_ = col;
    ...
    bool handleKeyDown(int key) override {
        ...
        case VK_BACK:
            ...
            if (cursorCol_ > 0) { lines_[cursorLine_].erase(--cursorCol_, 1); }
        ...
        case VK_DELETE:
            if (_hasSelection()) { _deleteSelection(); break; }
            if (cursorCol_ < (int)lines_[cursorLine_].size()) lines_[cursorLine_].erase(cursorCol_, 1);
            ...
            break;
        case VK_LEFT:
            if (!shift && _hasSelection()) { auto [s,e]=_normalizedSelection(); cursorLine_=s.line; cursorCol_=s.col; _clearSelection(); }
            else if (cursorCol_ > 0) cursorCol_--;
            else if (cursorLine_ > 0) { cursorLine_--; cursorCol_ = (int)lines_[cursorLine_].size(); }
            if (!shift) _clearSelection();
            break;
        case VK_RIGHT:
            if (!shift && _hasSelection()) { auto [s,e]=_normalizedSelection(); cursorLine_=e.line; cursorCol_=e.col; _clearSelection(); }
            else if (cursorCol_ < (int)lines_[cursorLine_].size()) cursorCol_++;
            else if (cursorLine_ < (int)lines_.size() - 1) { cursorLine_++; cursorCol_ = 0; }
            if (!shift) _clearSelection();
            break;
        case VK_UP:
            if (cursorLine_ > 0) { cursorLine_--; cursorCol_ = min(cursorCol_, (int)lines_[cursorLine_].size()); }
            if (!shift) _clearSelection();
            break;
        case VK_DOWN:
            if (cursorLine_ < (int)lines_.size() - 1) { cursorLine_++; cursorCol_ = min(cursorCol_, (int)lines_[cursorLine_].size()); }
            if (!shift) _clearSelection();
            break;
        case VK_HOME:
            cursorCol_ = 0;
    ...
    std::shared_ptr<TextAreaWidget> setPlaceholder(const std::string &ph)  { placeholder = ph;  return self_(); }
    std::shared_ptr<TextAreaWidget> setLineNumbers(bool v)   { showLineNumbers = v; gutterW_cached_ = -1; markNeedsLayout(); return self_(); }
    std::shared_ptr<TextAreaWidget> setWordWrap(bool v)      { wordWrap = v; markNeedsLayout(); return self_(); }
    std::shared_ptr<TextAreaWidget> setTabSpaces(int n)      { tabSpaces = n; return self_(); }
    std::shared_ptr<TextAreaWidget> setMaxLength(int n)      { maxLength = n; return self_(); }
    std::shared_ptr<TextAreaWidget> setFontSize(int s)       { fontSize = s; markNeedsLayout(); return self_(); }
    std::shared_ptr<TextAreaWidget> setWidth(int w)          { width = w; autoWidth = false; markNeedsLayout(); return self_(); }
    std::shared_ptr<TextAreaWidget> setHeight(int h)         { height = h; autoHeight = false; markNeedsLayout(); return self_(); }
    std::shared_ptr<TextAreaWidget> setFlex(int f)           { flex = f; return self_(); }
    std::shared_ptr<TextAreaWidget> setScrollbarSize(int s)  { sbV_.size = sbH_.size = s; return self_(); }
    std::shared_ptr<TextAreaWidget> setScrollbarColor(COLORREF c)       { sbV_.colorNormal = sbH_.colorNormal = c; return self_(); }
    std::shared_ptr<TextAreaWidget> setScrollbarHoverColor(COLORREF c)  { sbV_.colorHover  = sbH_.colorHover  = c; return self_(); }
    std::shared_ptr<TextAreaWidget> setScrollbarTrackColor(COLORREF c)  { sbV_.colorTrack  = sbH_.colorTrack  = c; return self_(); }
...
private:
    ...
    void _updateScrollContent() {
        ...
        if (!wordWrap) {
            int maxW = 0;
            for (auto &l : lines_) maxW = max(maxW, (int)l.size() * (fontSize / 2 + 1));
        ...
        if (!lines_[line].empty() && relX > 0) {
            ...
            if (hw) {
                ...
                for (int i = 1; i <= (int)lines_[line].size(); i++) {
                    SIZE sz;
                    GetTextExtentPoint32(hdc, lines_[line].c_str(), i, &sz);
    ...
    void _selectAll() {
        selAnchorLine_ = 0; selAnchorCol_ = 0;
    ...
    void _deleteSelection() {
        ...
        if (s.line == e.line) {
            ...
            lines_.erase(lines_.begin() + s.line + 1,
                         lines_.begin() + e.line + 1);
        }
        cursorLine_ = s.line; cursorCol_ = s.col;
        _clearSelection();
    }
    ...
    void _copyToClipboard() const {
        ...
        if (sel.empty()) return;
        HWND hw = _hwnd();
        if (!hw || !OpenClipboard(hw)) return;
        EmptyClipboard();
        HGLOBAL hg = GlobalAlloc(GMEM_MOVEABLE, sel.size() + 1);
        if (hg) { memcpy(GlobalLock(hg), sel.c_str(), sel.size() + 1); GlobalUnlock(hg); SetClipboardData(CF_TEXT, hg); }
        CloseClipboard();
    }
    ...
    void _cutToClipboard() {
        _copyToClipboard();
        if (_hasSelection()) _deleteSelection();
        else { lines_ = {""}; cursorLine_ = 0; cursorCol_ = 0; _clearSelection(); }
    }
    ...
    void _pasteFromClipboard() {
        ...
        // Keep anchor in sync after paste
        _clearSelection();
    }
...
public:
    ...
    bool handleTimer(UINT id) override {
        if (id == cursorTimerId_) { cursorVisible_ = !cursorVisible_; markNeedsPaint(); return true; }
        return false;
    }
    ...
    bool handleMouseMove(int mx, int my) override {
        ...
        if (newUp != upHovered_ || newDown != downHovered_) {
            upHovered_ = newUp; downHovered_ = newDown; markNeedsPaint();
        }
        return false;
    }

    bool handleMouseLeave() override {
        upHovered_ = downHovered_ = false; markNeedsPaint(); return false;
    }
    ...
    bool handleKeyDown(int key) override {
        switch (key) {
        case VK_UP:     _increment(); _commitImmediate(); return true;
        case VK_DOWN:   _decrement(); _commitImmediate(); return true;
        case VK_PRIOR:  value = min(maxValue, value + step * 10); _commitImmediate(); return true;
        case VK_NEXT:   value = max(minValue, value - step * 10); _commitImmediate(); return true;
        case VK_HOME:   value = minValue; _commitImmediate(); return true;
        case VK_END:    value = maxValue; _commitImmediate(); return true;
        case VK_RETURN: _commitEdit(); return true;
        case VK_ESCAPE:
            editing_    = false;
            editBuffer_ = _formatValue(value);
            markNeedsPaint();
        ...
        case VK_BACK:
            if (editing_ && editCursorPos_ > 0) {
                ...
                cursorVisible_ = true; markNeedsPaint();
            }
            return true;
        case VK_DELETE:
    ...
    std::shared_ptr<NumberInputWidget> setFlex(int f) { flex = f; return self_(); }
...
private:
    ...
    void _commitEdit() {
        if (!editBuffer_.empty()) {
            try { value = _clamp(_snap(std::stod(editBuffer_))); }
...
inline NumberInputWidgetPtr NumberInput(double minVal = 0.0,
                                        double maxVal = 100.0,
                                        double step   = 1.0) {
...
inline NumberInputWidgetPtr SpinBox(double minVal = 0.0,
                                    double maxVal = 100.0,
                                    double step   = 1.0) {
