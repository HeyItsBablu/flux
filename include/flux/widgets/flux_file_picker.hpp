...
public:
    ...
    std::shared_ptr<FilePickerWidget> setMode(FilePickerMode m) {
        mode_ = m; _autoLabel(); return self_();
    }
    ...
    std::shared_ptr<FilePickerWidget> setOnChanged(
            std::function<void(const std::string &)> fn) {
        onChanged_ = std::move(fn); return self_();
    }
    std::shared_ptr<FilePickerWidget> setOnMultiChanged(
            std::function<void(const std::vector<std::string> &)> fn) {
        onMultiChanged_ = std::move(fn); return self_();
    }
    std::shared_ptr<FilePickerWidget> setOnCancelled(
...
private:
    ...
    void _autoLabel() {
        if (!customLabel_.empty()) return;
        switch (mode_) {
        case FilePickerMode::Open:         customLabel_ = ""; break; // use _label()
        case FilePickerMode::OpenMultiple: customLabel_ = ""; break;
        case FilePickerMode::Save:         customLabel_ = ""; break;
        case FilePickerMode::Folder:       customLabel_ = ""; break;
        }
    }
    ...
    std::wstring _toWide(const std::string &s) const {
        if (s.empty()) return {};
    ...
    void _openFileDialog(HWND owner, bool multi) {
        ...
        std::wstring wTitle    = _toWide(title_.empty() ? "Open File" : title_);
        std::wstring wInitDir  = _toWide(initialDir_);
        std::wstring wDefExt   = _toWide(defaultExt_);
    ...
    void _openSaveDialog(HWND owner) {
        auto filter  = _buildFilterW();
        std::wstring wTitle    = _toWide(title_.empty() ? "Save File" : title_);
        std::wstring wInitDir  = _toWide(initialDir_);
        std::wstring wDefExt   = _toWide(defaultExt_);
        std::wstring wDefault  = _toWide(defaultFilename_);
    ...
    void _openFolderDialog(HWND owner) {
        ...
        std::wstring wTitle = _toWide(title_.empty() ? "Select Folder" : title_);
    ...
    void _setSinglePath(const std::string &p) {
        ...
        if (boundPath_)  boundPath_->set(p);
        if (boundPaths_) boundPaths_->set(paths_);
        if (onChanged_)  onChanged_(p);
        if (onMultiChanged_) onMultiChanged_(paths_);
        markNeedsPaint();
        _repaint();
    }
