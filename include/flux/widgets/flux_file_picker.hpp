...
// ============================================================================
// LINUX — forward declarations only
// The implementation lives in flux_file_picker_linux.cpp so that
// flux_window_linux.cpp can include this header without pulling in
// zenity/kdialog command-building code or requiring FileFilter to be
// defined before SDL2 headers.
// ============================================================================
...
// Payload heap-allocated by the background dialog thread, deleted here
...
// Async dialog functions — implemented in flux_file_picker_linux.cpp.
void linuxPickFileAsync(
    const std::string& title,
    const std::string& initialDir,
    const std::vector<FileFilter>& filters,
    std::function<void(std::string)> callback);

void linuxPickFilesAsync(
    const std::string& title,
    const std::string& initialDir,
    const std::vector<FileFilter>& filters,
    std::function<void(std::vector<std::string>)> callback);

void linuxSaveFileAsync(
    const std::string& title,
    const std::string& initialDir,
...
// ============================================================================
// PLATFORM BACKENDS (Win32 / Android — header-only as before)
// ============================================================================
...
private:
    ...
    void _openDialogWin32() {
        ...
        switch (mode_) {
        case FilePickerMode::Open:
            result = openFile(owner, title, initialDir_, defaultExt_, filters_); break;
        case FilePickerMode::OpenMultiple:
            result = openFiles(owner, title, initialDir_, defaultExt_, filters_); break;
        case FilePickerMode::Save:
            result = saveFile(owner, title, initialDir_, defaultFilename_, defaultExt_, filters_); break;
        case FilePickerMode::Folder:
            result = pickFolder(owner, title); break;
        }
    ...
    void _openDialogAndroid() {
        ...
        auto deliver = [self](std::vector<std::string> paths) {
            if (paths.empty()) { if (self->onCancelled_) self->onCancelled_(); return; }
        ...
        switch (mode_) {
        case FilePickerMode::Open:
            launchOpenIntent(title, filters_, false, std::move(deliver)); break;
        case FilePickerMode::OpenMultiple:
            launchOpenIntent(title, filters_, true, std::move(deliver)); break;
        case FilePickerMode::Save:
            launchSaveIntent(title, defaultFilename_, filters_, std::move(deliver)); break;
        case FilePickerMode::Folder:
            launchFolderIntent(title, std::move(deliver)); break;
        }
    }
#endif
    ...
    void _openDialogLinux() {
        ...
        switch (mode_) {
        case FilePickerMode::Open:
            linuxPickFileAsync(title, initialDir_, filters_,
                [self](std::string p) {
        ...
        case FilePickerMode::Save:
            ...
            break;
        case FilePickerMode::Folder:
            linuxPickFolderAsync(title, initialDir_,
                [self](std::string p) {
                    if (p.empty()) { if (self->onCancelled_) self->onCancelled_(); return; }
                    self->_setSinglePath(p);
                });
