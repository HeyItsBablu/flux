...
public:
  ...
  void positionChildren(int contentX, int contentY, int /*contentWidth*/,
    ...
    if (!vertical) {
      ...
      if (children.size() >= 1) {
        ...
        p0->x = contentX;
        p0->y = contentY;
        p0->positionChildren(p0->x + p0->paddingLeft, p0->y + p0->paddingTop,
      ...
      if (children.size() >= 2) {
        ...
        p1->y = contentY;
        p1->positionChildren(p1->x + p1->paddingLeft, p1->y + p1->paddingTop,
  ...
  bool handleMouseDown(int mx, int my) override {
    ...
    if (_hitDivider(mx, my)) {
      ...
      if (FluxUI *ui = FluxUI::getCurrentInstance())
        ui->captureMouseInput(); // was: SetCapture(hwnd)
  ...
  bool handleMouseUp(int mx, int my) override {
    ...
    if (FluxUI *ui = FluxUI::getCurrentInstance())
      ui->releaseMouseInput(); // was: ReleaseCapture()
  ...
  std::shared_ptr<SplitViewWidget>
  setOnRatioChanged(std::function<void(float)> cb) {
...
private:
  ...
  // Cached divider screen-space rect (set during computeLayout, used for
  // hit-test)
  ...
  int _totalSpace() const { return (vertical ? height : width) - dividerWidth; }

  // Size of pane 0 in the split axis
  int _pane0Size() const { return (int)(_totalSpace() * ratio); }

  // Size of pane 1 in the split axis
  int _pane1Size() const { return _totalSpace() - _pane0Size(); }
  ...
  bool _hitDivider(int mx, int my) const {
    int dx = x + _dividerX;
    int dy = y + _dividerY;
    return (mx >= dx && mx < dx + _dividerW && my >= dy && my < dy + _dividerH);
  }
  ...
  void _renderDivider(GraphicsContext &ctx) const {
    ...
    // Divider background
    painter.fillRect(dx, dy, _dividerW, _dividerH, col);
    ...
    if (resizable && dividerWidth >= 4) {
      ...
      if (!vertical) {
        ...
        int startY = dy + _dividerH / 2 - ((numDots - 1) * dotGap) / 2;
        for (int i = 0; i < numDots; ++i)
          painter.drawEllipse(cx - dotR, startY + i * dotGap - dotR, dotR * 2,
                              dotR * 2, gripCol, gripCol, 0);
      } else {
        int cy = dy + _dividerH / 2;
        int startX = dx + _dividerW / 2 - ((numDots - 1) * dotGap) / 2;
        for (int i = 0; i < numDots; ++i)
          painter.drawEllipse(startX + i * dotGap - dotR, cy - dotR, dotR * 2,
                              dotR * 2, gripCol, gripCol, 0);
      }
    }
  }
  ...
  void _updateCursor() const {
    if (FluxUI *ui = FluxUI::getCurrentInstance())
      vertical ? ui->setResizeCursorV() : ui->setResizeCursorH();
  }
