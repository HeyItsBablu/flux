...
#ifdef __ANDROID__
        // postToRenderThread has no render loop on Android — it runs immediately
        // on the calling thread, which may be the decode thread. Use
        // scheduleRebuild instead: it queues the widget and sets dirty,
        // then drainPendingRebuilds() picks it up on the render thread.
