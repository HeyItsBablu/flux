...
// GLuint is used for glTexture below — pull in the typedef via gl2.h.
// flux_platform.hpp already includes it, but be explicit here so this
// header compiles standalone in Android TUs.
#include <GLES2/gl2.h>
...
struct DecodedImage
{
    ...
    void clear() { pixels.clear(); width = height = 0; }
};
...
public:
...
#ifdef _WIN32
    struct Win32State;
    struct Win32StateDeleter { void operator()(Win32State*) const; };
    ...
    struct MacScaleCacheDeleter { void operator()(MacScaleCache*) const; };
    ...
    struct LinuxScaleCacheDeleter { void operator()(LinuxScaleCache*) const; };
    ...
    // Now a real GL texture object.  0 is the GL-spec invalid value.
    GLuint glTexture = 0;

    // Raw RGBA staging — written by decode thread, consumed on GL thread
    ...
    // CPU-side copy kept for context-loss re-upload
...
#ifdef __ANDROID__
            // Show the image as soon as pixels are ready even if Loaded
            // hasn't been set yet (pending promote)
            if (pending.ready() || glTexture != 0) {
                _platformRender(ctx, cx, cy, cw, ch);
                _applyTint(painter, cx, cy, cw, ch);
    ...
    {
        _loadAssetAsync(path); return self();
    }
    std::shared_ptr<ImageWidget> setUrl(const std::string& url, bool postToUI = true)
    {
...
private:
    ...
    void _loadAssetAsync(const std::string& path)
    {
        ...
        if (!f) { _setLoadState(ImageLoadState::Error); _scheduleRebuild(); return; }
        fseek(f, 0, SEEK_END); long sz = ftell(f); fseek(f, 0, SEEK_SET);
        if (sz <= 0) {
            fclose(f);
            _setLoadState(ImageLoadState::Error); _scheduleRebuild(); return;
        }
        ...
        if (!ok) _setLoadState(ImageLoadState::Error);
        _scheduleRebuild();
    ...
    void _loadFromMemorySync(const uint8_t* data, int len)
    {
        ...
        _platformPromote();
        if (!ok) { _setLoadState(ImageLoadState::Error); return; }
    ...
    bool _decodeIntoStaging(const uint8_t* data, int len)
    {
        ...
        bool ok = _platformStorePixels(raw, w, h);
        stbi_image_free(raw);
        return ok;
#endif
    }
    ...
    DestRect _calculateDestRect(int cx, int cy, int cw, int ch) const
    {
        ...
        case ImageFit::Contain:
            if (imgAspect > ctnAspect) { dw = (float)cw; dh = dw / imgAspect; }
            else                       { dh = (float)ch; dw = dh * imgAspect; }
            break;

        case ImageFit::Cover:
            if (imgAspect > ctnAspect) { dh = (float)ch; dw = dh * imgAspect; }
            else                       { dw = (float)cw; dh = dw / imgAspect; }
            break;
        ...
        case ImageFit::ScaleDown:
            if (imageWidth <= cw && imageHeight <= ch) {
                dw = (float)imageWidth; dh = (float)imageHeight;
        ...
        case Alignment::Center:       ox = freeX*0.5f; oy = freeY*0.5f; break;
        case Alignment::TopCenter:    ox = freeX*0.5f; oy = 0.f;        break;
        case Alignment::BottomCenter: ox = freeX*0.5f; oy = freeY;      break;
        case Alignment::CenterLeft:   ox = 0.f;        oy = freeY*0.5f; break;
        case Alignment::CenterRight:  ox = freeX;      oy = freeY*0.5f; break;
        case Alignment::Start:        ox = 0.f;        oy = 0.f;        break;
        case Alignment::End:          ox = freeX;      oy = freeY;      break;
        case Alignment::TopRight:     ox = freeX;      oy = 0.f;        break;
        case Alignment::BottomLeft:   ox = 0.f;        oy = freeY;      break;
        case Alignment::BottomRight:  ox = freeX;      oy = freeY;      break;
        default:                      ox = freeX*0.5f; oy = freeY*0.5f; break;
        }
                              ...
                              Painter& painter, int cx, int cy, int cw, int ch)
    {
        if (loadingBuilder) {
            ...
            if (w) { _renderSubWidget(ctx, fontCache, w, cx, cy, cw, ch); return; }
        }
        painter.fillRect(cx, cy, cw, ch, placeholderColor);
...
#else
    bool _platformDecode(const uint8_t* data, int len);           // stub
