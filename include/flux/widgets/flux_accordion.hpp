...
public:
    ...
    void expand(int idx) {
        ...
        if (panels_[idx].disabled) return;
        if (singleExpand_) _collapseAllExcept(idx);
        panels_[idx].expanded = true;
        animStates_[idx].animating = true;
        if (onChanged) onChanged(idx, true);
        markNeedsLayout();
        if (auto *ui = FluxUI::getCurrentInstance())
            ui->updateWidget(this);
    }
    ...
    void collapse(int idx) {
        ...
        animStates_[idx].animating = true;
        if (onChanged) onChanged(idx, false);
        markNeedsLayout();
        if (auto *ui = FluxUI::getCurrentInstance())
            ui->updateWidget(this);
    }
    ...
    void toggle(int idx) {
        if (idx < 0 || idx >= (int)panels_.size()) return;
        if (panels_[idx].expanded) collapse(idx);
        else                       expand(idx);
    }
    ...
    AccordionPanel *panelAt(int idx) {
        if (idx < 0 || idx >= (int)panels_.size()) return nullptr;
        return &panels_[idx];
    }
    ...
    void computeLayout(GraphicsContext &ctx, const BoxConstraints &constraints,
        ...
        // Pass 1 — measure every panel body. Always zero first (fix #3: no stale heights).
        for (int i = 0; i < (int)panels_.size(); i++) {
        ...
        for (int i = 0; i < (int)panels_.size(); i++) {
            ...
            if (p.expanded && p.body && bodyH > 0) {
                ...
                p.body->positionChildren(bodyX, bodyY,
                                         p.body->width, p.body->height);
            }
    ...
    bool handleMouseDown(int mx, int my) override {
        ...
        for (auto &p : panels_) {
            if (p.expanded && p.body) {
                if (findAndHandleMouseEvent(p.body.get(), mx, my,
                        [](Widget *w) { return w->handleMouseDown(0, 0); }))
                    return true;
            }
        }
        return false;
    ...
    std::shared_ptr<AccordionWidget> setSingleExpand(bool v) {
        singleExpand_ = v; return self_();
    }
    std::shared_ptr<AccordionWidget> setOnChanged(
            std::function<void(int, bool)> cb) {
        onChanged = std::move(cb); return self_();
    }
    ...
    std::shared_ptr<AccordionWidget> setBodyPadding(int p) {
        bodyPadding = p; markNeedsLayout(); return self_();
    }
    std::shared_ptr<AccordionWidget> setShowBorder(bool v) {
        showBorder = v; markNeedsLayout(); return self_();
    }
    std::shared_ptr<AccordionWidget> setShowSeparators(bool v) {
        showSeparators = v; markNeedsLayout(); return self_();
    }
    ...
    std::shared_ptr<AccordionWidget> setTitleFontSize(int s) {
        titleFontSize = s; markNeedsLayout(); return self_();
    }
...
private:
    // ── State ─────────────────────────────────────────────────────────────────
    struct AnimState { bool animating = false; int frame = 0; };
    ...
    void _collapseAllExcept(int keepIdx) {
        for (int i = 0; i < (int)panels_.size(); i++) {
            if (i != keepIdx && panels_[i].expanded) {
                panels_[i].expanded = false;
                animStates_[i].animating = true;
                if (onChanged) onChanged(i, false);
            }
        }
    }
