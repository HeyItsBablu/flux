...
public:
    ...
    // NOTE — this widget is a deliberate, scoped exception to the rule
    // (see roadmap Decision #1) that our own layout engine always owns
    // wrapping/positioning. On this backend it hands text WRAPPING to
    // the real browser <textarea> so it behaves like an actual HTML
    // textarea, per explicit request — Phase 3's future hydration-
    // mismatch checker should skip this widget's internal line geometry
    // rather than flag it. Widget x/y/width/height are still fully
    // computed by computeLayout() below on every backend, unchanged —
    // only what happens INSIDE that box differs.
                       ...
                       FontCache& fontCache) override
    {
        ...
        int charW = 8, lineH = 0;
        {
            ...
            Painter(ctx, this).measureText(sample, font, charW, lineH);
        }
    ...
    {
#if (defined(__EMSCRIPTEN__) && defined(FLUX_WEB_RENDERER_DOM)) || defined(FLUX_SSR)
        // Real <textarea> owns native caret/blink/selection — no timer,
        // no VirtualKeyboard::notify*(). Just keep isFocused in sync for
        // anything else reading it (the border color in _renderDom also
        // reflects it, layered under the element's own native :focus
        // ring the browser draws besides).
        isFocused = focused;
        return true;
#else
        isFocused = focused;
...
// ──────────────────────────────────────────────────────────────────────────────
...
#if (defined(__EMSCRIPTEN__) && defined(FLUX_WEB_RENDERER_DOM)) || defined(FLUX_SSR)
    // Native scrollTop, mirrored via onDomScrollChanged (below) whenever
    // the real <textarea> fires a 'scroll' event. Unrelated to the
    // hand-rolled scrollY field above, which the native element bypasses
    ...
    {
        inputValue = value;
        // Real element owns cursor/selection; cursorPos is not
        // authoritative here, same relationship TextInputWidget has.
        cursorPos = (int)inputValue.size();
        notifyStateBinding();
    }

    void onDomScrollChanged(int scrollTop) override
    {
        scrollTop_ = scrollTop;
        // Only the gutter's position depends on this.
        markNeedsPaint();
    }

    // Two independent nodes under one owner: the real <textarea>
    // (slot "textarea") and, only when line numbers are on, a purely
    // visual gutter overlay (slot "gutter") drawn on top of its left
    // padding strip.
    void _renderDom(IDomAdapter *adapter)
    {
        ...
        int effectiveGutter = showLineNumbers_ ? gutterWidth_ : 0;

        DomNodeHandle ta = fluxDomEnsureNode(this, "textarea", "textarea");
        fluxDomApplyRect(this, x, y, width, height, "textarea");
        adapter->setStyle(ta, "box-sizing", "border-box");
        adapter->setStyle(ta, "padding-left", px(paddingLeft + effectiveGutter));
        adapter->setStyle(ta, "padding-right", px(paddingRight));
        adapter->setStyle(ta, "padding-top", px(paddingTop));
        adapter->setStyle(ta, "padding-bottom", px(paddingBottom));
        adapter->setStyle(ta, "font-size", px(fontSize));
        adapter->setStyle(ta, "font-family", fontFamily);
        adapter->setStyle(ta, "border-radius", px(borderRadius));
        adapter->setStyle(ta, "border", "1px solid " + rgb(isFocused ? focusedBorderColor : unfocusedBorderColor));
        adapter->setStyle(ta, "background-color", "rgb(255,255,255)");
        adapter->setStyle(ta, "color", rgb(inputTextColor));
        adapter->setStyle(ta, "outline", "none");
        // "both" mirrors the canvas backend's own resize handle, which
        // dragged BOTH width and height (see handleMouseMove above).
        //
        // KNOWN HAZARD, not silently accepted: native resize sets the
        // element's own pixel size directly in the browser, bypassing
        // Widget::width/height entirely. Our layout engine still owns
        // those fields per Decision #1 and will overwrite them back to
        // rows*lineH / cols*charW on the next computeLayout() pass
        // triggered by ANY unrelated state change elsewhere in the app —
        // so a user's native resize can silently revert. Properly fixing
        // this needs a ResizeObserver forwarded through the adapter
        // (mirroring bindScrollEvent's shape) to sync the real size back
        // into Widget::width/height; flagged as a follow-up, not solved
        // here.
        adapter->setStyle(ta, "resize", resizable ? "both" : "none");
        // overflow:auto is required for the native resize handle to
        // ever appear, and gives the browser's own scrollbar for free —
        // the hand-drawn scrollbarTrackColor/scrollbarThumbColor theming
        // and drawScrollbar() are unused on this backend as a result.
        adapter->setStyle(ta, "overflow", "auto");
        adapter->setStyle(ta, "pointer-events", "auto");
        adapter->setStyle(ta, "z-index", "2");

        adapter->setAttr(ta, "placeholder", placeholder);
        if (maxLength >= 0)
            adapter->setAttr(ta, "maxlength", std::to_string(maxLength));
        adapter->setBoolProperty(ta, "readOnly", readOnly);
        adapter->setInputValue(ta, inputValue);
        adapter->bindInputEvents(ta, this);
        adapter->bindScrollEvent(ta, this);

        if (showLineNumbers_ && gutterWidth_ > 0)
        {
            // Numbers are assigned by HARD ('\n'-separated) line only —
            // the SAME simplification the canvas backend already used
            // (it never accounted for wrapped continuation lines in the
            // gutter either). With native soft-wrap enabled here (the
            // default, matching real <textarea> behavior), a long
            // wrapped hard-line will make gutter numbers drift out of
            // alignment with the wrapped visual lines beneath the first.
            // Pre-existing limitation, carried forward unchanged — not a
            // new regression, and not fixed here (would need either
            // disabling native wrap or measuring per-line wrap counts).
            DomNodeHandle gutter = fluxDomEnsureNode(this, "div", "gutter");
            fluxDomApplyRect(this, x + 1, y + 1, gutterWidth_, height - 2, "gutter");
            adapter->setStyle(gutter, "background-color", rgb(gutterBg_));
            adapter->setStyle(gutter, "overflow", "hidden");
            adapter->setStyle(gutter, "pointer-events", "none");
            adapter->setStyle(gutter, "z-index", "3"); // above the textarea (2)
            adapter->setStyle(gutter, "font-family", fontFamily);
            adapter->setStyle(gutter, "font-size", px(std::max(9, fontSize - 1)));
            adapter->setStyle(gutter, "color", rgb(gutterText_));
            adapter->setStyle(gutter, "text-align", "right");
            adapter->setStyle(gutter, "white-space", "pre");
            adapter->setStyle(gutter, "box-sizing", "border-box");
            adapter->setStyle(gutter, "padding-top", px(paddingTop));

            int lineH = std::max(1, fontSize + 4); // must track the CSS line-height text actually renders at
            adapter->setStyle(gutter, "line-height", px(lineH));
            // Tracks the real element's native scroll position (kept in
            // sync via onDomScrollChanged, above) instead of the
            // hand-rolled scrollY field, which the native element never
            // touches.
            adapter->setStyle(gutter, "transform", "translateY(" + std::to_string(-scrollTop_) + "px)");
            ...
            for (size_t i = 0; i <= inputValue.size(); ++i)
            {
                if (i == 0 || inputValue[i - 1] == '\n')
                {
                    ...
                    numbers += std::to_string(lineCount++);
                }
            }
            adapter->setText(gutter, numbers);
        }
    }
#endif
};
