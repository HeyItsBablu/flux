...
public:
    ...
    // ── Layout / render (zero-size anchor — stays a normal tree child
    //    purely so onDetach() fires when the app rebuilds) ────────────────
...
private:
    ...
    void _pump()
    {
        ...
        if (tickTimerId_ == 0)
        {
            auto *ui = FluxUI::getCurrentInstance();
            if (ui)
                tickTimerId_ = ui->setInterval(100, [this]
                                               { _tick(); });
        }
