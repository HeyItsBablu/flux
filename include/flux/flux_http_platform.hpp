...
inline void FluxWin_markNeedsPaint() {
    if (gFluxUIWindow) ::InvalidateRect(gFluxUIWindow, nullptr, FALSE);
}

inline void fluxDrainPendingMessages(HWND hwnd) {
    MSG msg;
    while (PeekMessage(&msg, hwnd,
                       WM_FLUX_HTTP_RESULT, WM_FLUX_HTTP_RESULT, PM_REMOVE)) {
...
inline void fluxPostToUIThread(HttpCallback callback, HttpResult result) {
    ...
    if (!PostMessage(gFluxUIWindow, WM_FLUX_HTTP_RESULT, 0,
...
// ─── macOS — GCD dispatch to main queue ─────────────────────────────────────
//
// Grand Central Dispatch is always available on macOS 10.6+ with no extra
// linkage (it's part of libSystem). dispatch_async(dispatch_get_main_queue())
// guarantees the callback fires on the main thread, which is where all
// FluxUI rendering and widget mutation happens.
//
// This header is a plain .hpp, not .mm, so we use the C GCD API
// (dispatch/dispatch.h) which is callable from C++ without ObjC.
//
...
inline void fluxPostToUIThread(HttpCallback callback, HttpResult result) {
    // Heap-allocate the payload; the block captures and owns it.
    auto* p = new FluxHttpPayload{std::move(callback), std::move(result)};
...
// No init/shutdown needed — GCD main queue always exists.
