...
public:
    ...
    void computeLayout(HDC hdc, int availableWidth, int availableHeight, FontCache &fontCache) override
    {
        ...
        if (!text.empty())
        {
            ...
            width = toggleWidth + 12 + textSize.cx; // 12px spacing between toggle and label
    ...
    void render(HDC hdc, FontCache &fontCache) override
    {
        ...
        int toggleY = y + paddingTop + (height - paddingTop - paddingBottom - toggleHeight) / 2;
        ...
        COLORREF currentTrackColor = interpolateColor(trackOffColor, trackOnColor, animationProgress);
    ...
    std::shared_ptr<ToggleWidget> setOnToggleChanged(std::function<void(bool)> callback)
    {
