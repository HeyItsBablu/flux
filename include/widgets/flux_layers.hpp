...
public:
  ...
  void addLayer(const std::string &name = "") {
    ...
    Layer lyr;
    lyr.name =
        name.empty() ? ("Layer " + std::to_string(layers_.size() + 1)) : name;
    lyr.visible = true;
    allocLayerFBOs(lyr);
  ...
  void deleteLayer(int idx) {
    ...
    int newActive =
        std::clamp(activeIdx_ >= idx ? activeIdx_ - 1 : activeIdx_, 0,
                   int(layers_.size()) - 2); // -2 because size shrinks by 1
  ...
  void moveLayerUp(int idx) {
    idx = clamped(idx);
    if (idx >= int(layers_.size()) - 1)
      return;
    std::swap(layers_[idx], layers_[idx + 1]);
    if (activeIdx_ == idx)
      activeIdx_++;
    else if (activeIdx_ == idx + 1)
      activeIdx_--;
    redirectToActive();
    compositeDirty_ = true;
    fire();
  ...
  void setLayerVisible(int idx, bool visible) {
    idx = clamped(idx);
    if (layers_[idx].visible == visible)
      return;
  ...
  void resize(int w, int h) override {
    ...
    for (auto &lyr : layers_) {
      ...
      if (bw > 0 && bh > 0)
        blitFBOs(lyr.committedFBO, nFBO, bw, bh);
...
private:
  ...
  void swapInDisplay() {
    ...
    swapActiveFBOs(dispCFBO_, dispCTex_, activeSFBO_,
                   activeSTex_, // keep same scratch
  ...
  void swapInActive() {
    GLuint outCF, outCT, outSF, outST;
    swapActiveFBOs(activeCFBO_, activeCTex_, activeSFBO_, activeSTex_, outCF,
                   outCT, outSF, outST);
  ...
  void redirectToActive() {
    ...
    // up-to-date values.
    activeCFBO_ = lyr.committedFBO;
    activeCTex_ = lyr.committedTex;
    activeSFBO_ = lyr.scratchFBO;
    activeSTex_ = lyr.scratchTex;

    GLuint d1, d2, d3, d4;
    swapActiveFBOs(activeCFBO_, activeCTex_, activeSFBO_, activeSTex_, d1, d2,
                   d3, d4);
  }
  ...
  void buildComposite(bool force = false) {
    ...
    for (int li = 0; li < int(layers_.size()); li++) {
      ...
      glBindFramebuffer(GL_READ_FRAMEBUFFER, lyr.committedFBO);
      ...
      if (hasScratch) {
        ...
        glBindFramebuffer(GL_READ_FRAMEBUFFER, scratchFBOHandle());
  ...
  static void clearFBOColor(GLuint fbo, int w, int h, uint8_t r, uint8_t g,
                            uint8_t b, uint8_t a) {
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
    glViewport(0, 0, w, h);
  ...
  void clearLayerContent(Layer &lyr, uint8_t r, uint8_t g, uint8_t b,
                         uint8_t a) {
    int cw = canvasWidth(), ch = canvasHeight();
  ...
  void flushUndoRedo() { flushUndoRedoPublic(); }
...
inline std::shared_ptr<CanvasWidget> LayeredCanvas(int viewW, int viewH,
