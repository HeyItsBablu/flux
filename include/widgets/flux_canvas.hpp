...
public:
  ...
  CanvasContext &quadraticCurveTo(float cpx, float cpy, float x, float y) {
    ...
    c.x0 = cpx; c.y0 = cpy; c.x1 = x; c.y1 = y;
    path_.push_back(c);
    return *this;
  }
  ...
  CanvasContext &ellipse(float cx, float cy, float rx, float ry, float rotation,
    ...
    for (int i = 0; i <= segs; i++) {
      ...
      if (first) { moveTo(px, py); first = false; }
      else lineTo(px, py);
    }
    return *this;
  }
  ...
  CanvasContext &fillRect(float x, float y, float w, float h) {
    setColor(st_.top().fillColor);
    glBegin(GL_QUADS);
    emitV(x, y); emitV(x + w, y); emitV(x + w, y + h); emitV(x, y + h);
    glEnd();
    return *this;
  }
...
private:
  ...
  void setColor(const RGBA &c) {
    glColor4f(c.r, c.g, c.b, c.a * st_.top().alpha);
  }
  ...
  void rasterisePath(bool doFill) {
    ...
    if (doFill) {
      for (auto &sp : subs) {
        ...
        for (auto &v : sp) glVertex2f(v.x, v.y);
        glEnd();
      }
    } else {
      for (auto &sp : subs) {
  ...
  TextTex makeTextTex(const std::string &text, bool filled) {
    ...
    int tw = max(1, (int)std::ceil(bounds.Width) + 2),
        th = max(1, (int)std::ceil(bounds.Height) + 2);
...
public:
  ...
  //
  // Bind any number of State<T> values to the canvas.
  // Whenever a bound state changes, the canvas redraws automatically.
  //
  // Usage:
  //   canvas->bindState(dotX)->bindState(dotY)->bindState(dragging);
  //
  template <typename T>
  std::shared_ptr<CanvasWidget> bindState(State<T> &state) {
    state.bindProperty(
        shared_from_this(),
        [](Widget *, const T &) {
            // no-op applier — markNeedsPaint override handles the redraw
        },
        false // repaint only, no layout needed
    );
    return ptr();
  }
  ...
  // ── Reactive repaint ──────────────────────────────────────────────────────
  //
  // Override markNeedsPaint so that when State::bindProperty fires it after a
  // state change, we post a deferred WM_USER+1 to the GL child window instead
  // of rendering synchronously mid-set().
  //
  // The repaintPending_ flag deduplicates multiple rapid set() calls (e.g.
  // dragging.set(true) + dotX.set(mx) + dotY.set(my) in one mouse handler)
  // so only a single renderGL() runs after all state mutations complete.
  //
...
private:
  ...
  // Deduplication: true while a WM_USER+1 is pending in the message queue.
  // Cleared when the message is processed and renderGL() is about to run.
  // Safe without a mutex — everything runs on the single Win32 message thread.
  ...
  static LRESULT CALLBACK ChildProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    // ── Deferred repaint (posted by markNeedsPaint) ───────────────────────
    case WM_USER + 1:
      if (self) {
        self->repaintPending_ = false;  // clear flag before rendering so any
                                        // set() calls inside drawFn can re-arm
        self->renderGL();
      }
      return 0;
...
// ── Drag a dot ───────────────────────────────────────────────────────────────
//
// bindState() is the idiomatic way to wire states to a canvas.
// Each set() call posts at most ONE deferred WM_USER+1, so three rapid
// set() calls in onMouseDown coalesce into a single renderGL() invocation.
...
auto canvas = Canvas(700, 480)
    ...
    ->onMouseDown([&](int mx, int my) {
        dragging.set(true);   // all three set() calls coalesce into one redraw
        dotX.set(mx);
        dotY.set(my);
    })
