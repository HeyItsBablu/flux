...
private:
    ...
    void updateScrollbar() {
        ...
        scrollbarThumbY   = (int)(scrollRatio *
                            (viewportHeight - scrollbarThumbHeight));
    }
    ...
    bool isMouseOverScrollbarThumb(int mx, int my) const {
        ...
        return mx >= sx && mx < x + width &&
               my >= top && my < top + scrollbarThumbHeight;
    }

    void renderScrollbar(HDC hdc) {
        // Track
        HBRUSH trackBrush = CreateSolidBrush(scrollbarTrackColor);
        RECT   trackRect  = { x + width - scrollbarWidth, y,
                              x + width, y + height };
        ...
        RECT   thumbRect  = { x + width - scrollbarWidth,
                              y + scrollbarThumbY,
                              x + width,
...
public:
    explicit GridViewBuilder(State<std::vector<T>> &state)
        : boundState(&state)
    {
        lastItemCount = (int)state.get().size();
    ...
    ~GridViewBuilder() override { if (isDraggingScrollbar) ReleaseCapture(); }
    ...
    std::shared_ptr<GridViewBuilder<T>> setSpacingH(int s) {
        spacingH = s; markNeedsLayout(); return self;
    }
    std::shared_ptr<GridViewBuilder<T>> setSpacingV(int s) {
        spacingV = s; markNeedsLayout(); return self;
    }
    std::shared_ptr<GridViewBuilder<T>> setSpacing(int s) {
        spacingH = spacingV = s; markNeedsLayout(); return self;
    }
                       ...
                       FontCache &fontCache) override
    {
        ...
        // Fill available space (same as ListViewBuilder)
        if (autoHeight || height > availableHeight) height = availableHeight;
        if (autoWidth  || width  > availableWidth)  width  = availableWidth;
        ...
        int totalH = spacingH * (cols - 1);
        int cellW = cols > 1 ? (contentW - spacingH * (cols - 1)) / cols
                             : contentW;
        ...
        for (int i = 0; i < (int)children.size(); i++) {
            int row = i / cols;
            children[i]->computeLayout(hdc, cellW, viewportHeight, fontCache);
        ...
        // Re-measure with correct scrollbar width now that isScrollable is set
        sbW      = isScrollable ? scrollbarWidth : 0;
        contentW = width - paddingLeft - paddingRight - sbW;
        cols     = resolvedColumnCount(contentW);
        cellW    = cols > 1 ? (contentW - spacingH * (cols - 1)) / cols
                            : contentW;

        // Store for positionChildren
        _cols       = cols;
        _cellW      = cellW;
    ...
    bool handleMouseDown(int mx, int my) override {
        ...
        if (mx < sbX) return false;  // not in scrollbar column
    ...
    bool handleMouseMove(int mx, int my) override {
        if (isDraggingScrollbar) {
            ...
            scrollOffset      = dragStartOffset + (int)(ratio * (contentHeight - viewportHeight));
            clampScrollOffset();
            updateScrollbar();
            repositionChildren();
            markNeedsPaint();
    ...
    void render(HDC hdc, FontCache &fontCache) override {
        ...
        HRGN clip = CreateRectRgn(x + paddingLeft, y + paddingTop,
                                  clipRight, y + height - paddingBottom);
        SelectClipRgn(hdc, clip);
...
public:
                       ...
                       FontCache &fontCache) override
    {
        ...
        // Resolve actual column count
        int cols = resolvedColumnCount(contentWidth);
        if (cols < 1) cols = 1;

        // Cell width — same for every column
        int totalSpacingH = spacingH * (cols - 1);
        int cellW = (contentWidth - totalSpacingH) / cols;

        // Lay out each child at its cell width, collect row heights
        ...
        for (int i = 0; i < (int)children.size(); i++) {
            ...
            // Stretch fills the full cell width; otherwise let child size itself
            int childAvailW = (crossAlignment == Alignment::Stretch)
                              ? cellW
                              : min(cellW, availableWidth);

            child->computeLayout(hdc, childAvailW, availableHeight, fontCache);
        ...
        if (autoWidth)  width  = availableWidth; // grid fills available width
...
inline GridWidgetPtr GridFixedWidthFromList(int cellWidth,
                                            const std::vector<WidgetPtr> &items) {
    auto w = std::make_shared<GridWidget>();
    w->fixedCellWidth = cellWidth;
    for (auto &item : items)
